...
#define MEM_CP_FLASH_SIZE  __MB__(10)
 ...
 *    (58K)       |     未使用
 * -------------- |
 * 0x20050000     |     接在AP核上的SRAM(总计384K)
 *    (100K)      |     AP-SRAM
 *    (28K)       |     LUNA CODE & DATA
 * --------------------------------------------
 * 0x20070000     |     AP算法使用(HardCode)
 *     |          |
 *   (256K)       |     ALGO
 *     |          |
 * --------------------------------------------
...
/* 独立给算法使用 */
#define MEM_APRAM_BASE  0x20070000
#define MEM_APRAM_SIZE  (__KB__(256))
...
#define MEM_AP_SRAM_BASE  0x20050000
#define MEM_AP_SRAM_SIZE  (__KB__(100))

#define MEM_LUNA_BASE  ((MEM_AP_SRAM_BASE) + (MEM_AP_SRAM_SIZE))
#define MEM_LUNA_SIZE  (__KB__(28))

#define MEM_IPC_BASE  (MEM_CP_SRAM_BASE + MEM_CP_SRAM_SIZE)
