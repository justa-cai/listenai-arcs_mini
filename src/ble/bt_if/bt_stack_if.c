...
static void bt_stack_reset_cmp(uint16_t status)
{
    ...
    if(p_mac_addr){
        memcpy(bt_cfg->addr.addr, p_mac_addr, 6);
        LISA_LOGD(TAG, "get mac from mac_manager");
    }

    LISA_LOGI(TAG, "bt reset cmp, BLE mac address: %02x:%02x:%02x:%02x:%02x:%02x",
                        bt_cfg->addr.addr[0],bt_cfg->addr.addr[1],bt_cfg->addr.addr[2],
                        bt_cfg->addr.addr[3],bt_cfg->addr.addr[4],bt_cfg->addr.addr[5]);
...
static void bt_stack_actv_start_ind(uint8_t type, uint8_t actv_id, int16_t status)
{
    switch(type)
    {
        ...
        case GAPM_ACTV_TYPE_PER_SYNC :
            {
                LISA_LOGD(TAG, "bt actv start:%d,%d", type, actv_id);
            }
            break;
        default : break;
    }
...
static void bt_stack_actv_stop_ind(uint8_t type, uint8_t actv_id, int16_t status)
{
    switch(type)
    {
        ...
        case GAPM_ACTV_TYPE_PER_SYNC :
            {
                LISA_LOGD(TAG, "bt actv stop:%d,%d", type, actv_id);
            }
            break;
        default : break;
    }
...
uint8_t bt_send_schedule_notify(void)
{
    ...
    else
    {
        ...
        if(s_cnt++ >= 200)
        {
            LISA_LOGD(TAG, "n o:%d!", stack_env->bt_notify_pending_num);
            s_cnt = 0;
        }
    }
...
{
    LISA_LOGD(TAG, "bt if init:%d", type);
    bt_platform_init(0);
    ...
    if(event->msg_body)
    {
        switch(event->msg_body->msg_id)
        {
            ...
            case BT_OS_DATA_SEND_CNF_EVT :
            {
                ...
                //CLOGD("bt free rcv data");
                app_bt_data_free(pkt_info->data);
            }break;
#endif
            default:
            {
                LISA_LOGD(TAG, "bt if msg err,id:%d", event->msg_body->msg_id);
            }break;
        }
    }
    else
    {
        LISA_LOGD(TAG, "bt if body err %x", event->msg_body);
    }
    // check bt task.
    ble_task_execute();
    return msg_free;
...
{
    LISA_LOGE(TAG, "nvs_get err, not support");
    return (NVDS_FAIL);
}
uint8_t bt_stack_nvs_set(uint8_t param_id, uint8_t length, uint8_t *buf)
{
    LISA_LOGE(TAG, "nvs_set err, not support");
    return (NVDS_FAIL);
}
uint8_t bt_stack_nvs_del(uint8_t param_id)
{
    LISA_LOGE(TAG, "nvs_del err, not support");
    return (NVDS_FAIL);
}
#else
uint8_t bt_stack_nvs_get(uint8_t param_id, uint8_t * lengthPtr, uint8_t *buf)
{
    ...
    *lengthPtr = len;
    LISA_LOGD(TAG, "nvs_get,id:0x%x, ret:%d", param_id, ret);
    return ret;
}
uint8_t bt_stack_nvs_set(uint8_t param_id, uint8_t length, uint8_t *buf)
{
    ...
    LISA_LOGD(TAG, "nvs_set,id:0x%x, ret:%d", param_id, ret);
    return ret;
}
uint8_t bt_stack_nvs_del(uint8_t param_id)
{
    LISA_LOGD(TAG, "nvs_del,id:0x%x", param_id);
