...
int romfs_init(struct romfs **p_fs, const void *base_addr, uint32_t size)
{
    ...
    if (fs == NULL) {
        LISA_LOGE(TAG, "romfs malloc failed");
        return -1;
    }
    ...
    *p_fs = fs;
    ...
    if (sb->word0 != ROMSB_WORD0 || sb->word1 != ROMSB_WORD1) {
        LISA_LOGE(TAG, "Invalid romfs super block");
        return -1;
    }

    img_size = be32_to_cpu(sb->size);
    if (img_size < ROMFH_SIZE || img_size > size) {
        LISA_LOGE(TAG, "Invalid romfs image size");
        return -1;
    }
    ...
    if (img_checksum != 0) {
        LISA_LOGE(TAG, "Invalid romfs image checksum, expected: %08x, actual: %08x", 0U, img_checksum);
        return -1;
    }
    ...
    if (!romfs_inode_is_valid(fs, fs->root) || romfs_node_type(fs->root) != ROMFH_DIR) {
        LISA_LOGE(TAG, "Invalid romfs root inode type");
        return -1;
    }

    LISA_LOGI(TAG, "romfs init success, volume: %s, size: %d, addr: %p", sb->name, img_size, sb);
...
int romfs_dir_iter_start(struct romfs *fs, const char *dir_path, struct romfs_dir_iter *iter)
{
    ...
    memset(iter, 0, sizeof(struct romfs_dir_iter));
