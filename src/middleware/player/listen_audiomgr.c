...
int listen_audiomgr_register_channel(listen_audiomgr_t *handle,
                                     ...
                                     int *capture_ids,
                                     int capture_count,
                                     void (*on_focus_change)(focus_state_e, int))
{
    if (handle == NULL) {
 ...
 * @brief 设置通道焦点状态
 */
static void _set_channel_focus(listen_audiomgr_t *handle, int id, focus_state_e state, int by_which)
{
    ...
    LISA_LOGI(TAG, "Channel focus: id=%d, state=%s, by_which=%d",
              id, listen_audiomgr_get_state_name(state), by_which);

    // 调用回调
    if (channel->on_focus_change != NULL) {
        channel->on_focus_change(state, by_which);
    }
}
...
void listen_audiomgr_release_channel(listen_audiomgr_t *handle, int id)
{
    ...
    // 如果无前景但有背景，背景升为前景
    if (handle->foreground_id < 0 && handle->background_id >= 0) {
