...
static void common_btn_evt_cb(void *arg)
{
    ...
    if (btn_ctx.cb) {
        lisa_btn_info_t info = {
            .id = (lisa_btn_id_t)btn->id,
            .click_count = btn->click_cnt,
            .press_duration_ms = btn->scan_cnt * (1000 / FLEX_BTN_SCAN_FREQ_HZ),
            .user = btn_ctx.user,
        };
        btn_ctx.cb((lisa_btn_event_t)btn->event, &info);
    }
}
...
void lisa_btn_init(lisa_btn_cb_t cb, void *user)
{
    ...
    btn_ctx.btns[i].long_press_start_tick = FLEX_MS_TO_SCAN_CNT(3000);
    btn_ctx.btns[i].long_hold_start_tick = FLEX_MS_TO_SCAN_CNT(3000);
