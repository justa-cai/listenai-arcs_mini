...
{
	(*focus_inst) = (focus_state_t *)lisa_mem_alloc(sizeof(focus_state_t));
...
static int _capture_on(channel_type_e channel_name, focus_state_t *focus_state)
{
	switch (channel_name) {
		case AIP:
			...
			break;
		case TTS:
			if (focus_state->m_channel == ALERT) {
				return 1;
			}
			break;
		case ALERT:
...
static focus_state_t *_get_focus_state(listen_audiomgr_t *handle, channel_type_e name)
{
	switch (name) {
		case AIP:
			return handle->m_aip;
...
static void _set_channel_focus(listen_audiomgr_t *handle, channel_type_e name, focus_state_e state, channel_type_e by_which)
{
	focus_state_t *focus_state = _get_focus_state(handle, name);
	if (focus_state == NULL) {
		LISA_LOGE(TAG, "channel %d is not exist", name);
		return;
	}
...
void listen_audiomgr_acquire_channel(listen_audiomgr_t *handle, channel_type_e channel_name)
{
	LISA_LOGD(TAG, "%d acquire channel", channel_name);
	focus_state_t *channel = _get_focus_state(handle, channel_name);
	if (channel == NULL) {
		LISA_LOGE(TAG, "channel %d is not exist", channel_name);
		return;
	}
	if (handle->m_foreground_channel != NULL) {
		...
		if (_capture_on(channel_name, handle->m_foreground_channel)) {
			...
			listen_audiomgr_acquire_channel(handle, channel_name);
...
void listen_audiomgr_release_channel(listen_audiomgr_t *handle, channel_type_e channel_name)
{
	LISA_LOGD(TAG, "%d release channel", channel_name);
