...
int lisaui_data_init(void)
{
    ...
    // 初始化待机文本配置
    _userdata.standby_texts.is_enabled = false;  // 默认不启用云端文本
    _userdata.standby_texts.interval_ms = 5000;  // 默认5秒间隔
    _userdata.standby_texts.text_count = 0;      // 默认没有云端文本
    memset(_userdata.standby_texts.texts, 0, sizeof(_userdata.standby_texts.texts));
...
{
    if (!texts || count == 0 || count > LISAUI_USERDATA_STANDBY_TEXT_MAX_COUNT) {
        return -1;
    }
    ...
    // 复制新文本
    _userdata.standby_texts.text_count = count;
    for (uint32_t i = 0; i < count; i++) {
        if (texts[i]) {
            strncpy(_userdata.standby_texts.texts[i], texts[i], 
                   LISAUI_USERDATA_STANDBY_TEXT_MAX_LENGTH - 1);
            _userdata.standby_texts.texts[i][LISAUI_USERDATA_STANDBY_TEXT_MAX_LENGTH - 1] = '\0';
        }
    }
    
    // 设置配置参数
    _userdata.standby_texts.interval_ms = interval_ms > 0 ? interval_ms : 5000;
    _userdata.standby_texts.is_enabled = enable;
...
int lisaui_userdata_clear_standby_texts(void)
{
    ...
    _userdata.standby_texts.is_enabled = false;
    _userdata.standby_texts.text_count = 0;
    _userdata.standby_texts.interval_ms = 5000;
    memset(_userdata.standby_texts.texts, 0, sizeof(_userdata.standby_texts.texts));
