...
static void lsc_stream_text_request_thread(void *arg)
{
	...
	if (td_arg->queue == NULL || td_arg->busy_lock == NULL) {
		LOGI("request thread arg invalid");
		return;
	}
	...
	while (1) {
		...
		err = lisa_queue_pop(td_arg->queue, &ctx, sizeof(struct lsc_stream_text_request_ctx *),
				     LISA_OS_WAIT_FOREVER);
		if (err) {
			LOGE("lisa queue pop error: %d", err);
			continue;
		}

		set_curr_ctx(ctx);

		lisa_mutex_lock(td_arg->busy_lock, LISA_OS_WAIT_FOREVER);

		LOGI("request start, url:%s", ctx->url);
		err = lsc_stream_text_request_start(ctx, 120);
		lsc_stream_text_request_delete(ctx);
		set_curr_ctx(NULL);
		if (err) {
			LOGE("request error: %d", err);
		} else {
			LOGI("request done");
		}
...
int lsc_stream_text_request_thread_init(void)
{
	...
	if (queue == NULL) {
		LOGE("lsc stream queue creation failed");
		goto init_err_exit;
	}

	request_busy_lock = lisa_mutex_create();
	if (request_busy_lock == NULL) {
		LOGE("lsc stream busy lock creation failed");
		goto init_err_exit;
	}

	exit_sem = lisa_semaphore_create(1);
	if (exit_sem == NULL) {
		LOGE("lsc stream exit_sem creation failed");
		goto init_err_exit;
	}

	lock = lisa_mutex_create();
	if (lock == NULL) {
		LOGE("lsc stream lock creation failed");
		goto init_err_exit;
	}
	...
	if (thread == NULL) {
		LOGE("lsc stream thread creation failed");
		goto init_err_exit;
	}
...
{
	/* lock */
	lisa_mutex_lock(lock, LISA_OS_WAIT_FOREVER);
	...
	while (lisa_queue_pop(queue, &abort_ctx, sizeof(struct lsc_stream_text_request_ctx), 0) == 0) {
		LOGI("stream text drop request url:%s", abort_ctx.url);
		if (abort_ctx.cb) {
	...
	if (lisa_mutex_lock(request_busy_lock, 0) != 0) {
		...
		if (curr) {
			...
			if (lsc_stream_text_request_abort(curr)) {
				LOGE("send abort sem failed");
			}
		}

		/* wait request thread idle */
		lisa_mutex_lock(request_busy_lock, LISA_OS_WAIT_FOREVER);
