...
/**
 * @brief I2C/SCCB硬件配置结构体
 */
 typedef struct {
    uint8_t  slave_addr;        // 从设备地址
    uint32_t clock_freq_hz;     // 时钟频率
    uint8_t  addr_width;        // 地址位宽 (8位或16位)
    bool     enable_pullup;     // 是否使能上拉电阻
...
#define CAMERA_LOG(fmt, ...)                      CLOG("CAMERA: "fmt"\r\n", ##__VA_ARGS__)
#define CAMERA_LOGD(fmt, ...)                     CLOGD("CAMERA: "fmt"\r\n", ##__VA_ARGS__)
#define CAMERA_LOGI(fmt, ...)                     CLOGI("CAMERA: "fmt"\r\n", ##__VA_ARGS__)
#define CAMERA_LOGE(fmt, ...)                     CLOGE("CAMERA: "fmt"\r\n", ##__VA_ARGS__)
#define CAMERA_LOGW(fmt, ...)                     CLOGW("CAMERA: "fmt"\r\n", ##__VA_ARGS__)
