...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    case PIXFORMAT_RGB565:
        ret = set_reg_bits(sensor, 0x12, 2, 1, 1);
        break;
    case PIXFORMAT_RAW:
        ret = set_reg_bits(sensor, 0x12, 0, 3, 0x4);
        break;
    case PIXFORMAT_YUV422:
    case PIXFORMAT_GRAYSCALE:
       ret = set_reg_bits(sensor, 0x12, 2, 1, 0);
        break;
    default:
        return -1;
    }
...
{
    // int16_t x = CAMERA_READ_REG8(sensor->slv_addr, 0x17);
    *w = CAMERA_READ_REG8(sensor->slv_addr, 0x18) << 1;
    // int16_t x = CAMERA_READ_REG8(sensor->slv_addr, 0x19);
    *h = CAMERA_READ_REG8(sensor->slv_addr, 0x1a) << 1;

    // LOGI("get_window: w:%d, h:%d", *w, *h);
