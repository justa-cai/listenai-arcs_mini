...
__STATIC_API__ string_buffer_t resize_buffer(string_buffer_t sb, uint_t __allocs)
{
    ivAssert(0);
    return NULL;
...
FRISO_API string_buffer_t new_string_buffer()
{
    ...
    memset(sb->buffer, 0, sizeof(sb->buffer));
    sb->length = 0;
    sb->allocs = WS_INPUT_TEXT_MAX_LEN;
...
FRISO_API string_buffer_t new_string_buffer_with_string(fstring str)
{
    ...
    if (strlen(str) > WS_INPUT_TEXT_MAX_LEN)
    {
        ivAssert(0);
        return NULL;
    }

    sb = (string_buffer_t)FRISO_MALLOC(sizeof(string_buffer_entry));
    if (NULL == sb)
    {
        ivAssert(0);
        return NULL;
...
FRISO_API void set_string_buffer_with_string(string_buffer_t sb, fstring str)
{
    if (strlen(str) > WS_INPUT_TEXT_MAX_LEN)
    {
        ivAssert(0);
...
FRISO_API void string_buffer_append(string_buffer_t sb, fstring __str)
{
...
FRISO_API void string_buffer_append_char(string_buffer_t sb, char ch)
{
    // check the necessity to resize the buffer.
 ...
 */
FRISO_API fstring string_buffer_remove( // no use
    string_buffer_t sb,
    uint_t idx,
    uint_t length)
{
    ...
    return sb->buffer; //此处不能返回sb->buf，待修改
}
 ...
 */
FRISO_API string_buffer_t string_buffer_trim(string_buffer_t sb) // no use
{
    // resize the buffer.
...
{
    fstring buffer = (fstring)FRISO_MALLOC(strlen(sb->buffer) + 1);
    memcpy(buffer, sb->buffer, sb->length);
    FRISO_FREE(sb);
    return buffer;
}
 ...
 */
// FRISO_API string_split_t new_string_split(fstring delimiter, fstring source )
// {
...
FRISO_API void string_split_reset(string_split_t sst, fstring delimiter, fstring source)
{
...
FRISO_API void string_split_set_source(string_split_t sst, fstring source)
{
    sst->source = source;
    sst->srcLen = strlen(source);
    sst->idx = 0;
...
FRISO_API void string_split_set_delimiter(string_split_t sst, fstring delimiter)
{
    sst->delimiter = delimiter;
    sst->delLen = strlen(delimiter);
    sst->idx = 0;
 ...
 */
FRISO_API fstring string_split_next(string_split_t sst, fstring __dst)
{
    uint_t i, _ok;
    fstring _dst = __dst;

    // check if reach the end of the fstring
    if (sst->idx >= sst->srcLen)
    {
        // ivAssert(0);
        return NULL;
    }
    ...
    {
        _ok = 1;
        for (i = 0; i < sst->delLen && (sst->idx + i < sst->srcLen); i++)
        {
