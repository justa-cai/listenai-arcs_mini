    ...
    friso_task_t task)
{
    ...
    {
        return unicode_cjk_string(task->unicode);
    }
    ...
    friso_task_t task)
{
    ...
    if (u >= 224 && u <= 476)
    {
        ...
        uint_t pnPinyinWithTone[] = {224,
                                     ...
                                     474,
                                     476};
        for (int i = 0; i < sizeof(pnPinyinWithTone) / sizeof(pnPinyinWithTone[0]); i++)
        {
            if (u == pnPinyinWithTone[i])
                return FRISO_EN_LETTER; //拼音有先返回字母类型
        }
    }
    if (u > 126 || u < 32)
        return FRISO_EN_UNKNOW;
    if (u == 32)
