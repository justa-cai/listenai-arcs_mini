...
lisa_err_t lisa_queue_push(lisa_queue_t *queue, void *item, uint32_t item_size, int32_t wait)
{
	...
	if (xPortIsInsideInterrupt()) {
		...
		ret = xQueueSendFromISR((QueueHandle_t)queue->handle, item, &higher_priority_task_woken);
...
lisa_err_t lisa_queue_push_front(lisa_queue_t *queue, void *item, uint32_t item_size, int32_t wait)
{
	...
	if (xPortIsInsideInterrupt()) {
		BaseType_t higher_priority_task_woken = pdFALSE;
		ret = xQueueSendToFrontFromISR((QueueHandle_t)queue->handle, item, &higher_priority_task_woken);
		if (ret == pdPASS && higher_priority_task_woken == pdTRUE) {
			portYIELD_FROM_ISR(higher_priority_task_woken);
		}
	} else {
		...
		ret = xQueueSendToFront((QueueHandle_t)queue->handle, item, wait_tick);
	}
	if (ret != pdPASS) {
		return LISA_FAIL;
	}
...
lisa_err_t lisa_queue_pop(lisa_queue_t *queue, void *item, uint32_t item_size, int32_t wait)
{
	...
	if (xPortIsInsideInterrupt()) {
		BaseType_t higher_priority_task_woken = pdFALSE;
		ret = xQueueReceiveFromISR((QueueHandle_t)queue->handle, item, &higher_priority_task_woken);
		if (ret == pdPASS && higher_priority_task_woken == pdTRUE) {
			portYIELD_FROM_ISR(higher_priority_task_woken);
		}
	} else {
