 ...
 * @date 2025-12-05
 * 
 * @copyright Copyright (c) 2018 - 2025 shenzhen listenai co., ltd.
 * 
...
//  - value: 0x0AAB
//      name: Anhui Listenai Co., Ltd.
#define BLE_COMP_ID_LISTENAI    0xAB, 0x0A
...
uint8_t app_ble_adv_start(uint8_t adv_id, uint8_t adv_type)
{
    ...
    LISA_LOGI(TAG, "adv start:%d,%d", adv_id, adv_type);
...
uint8_t app_ble_adv_stop(uint8_t adv_id)
{
    ...
    LISA_LOGI(TAG, "adv stop:%d", adv_id);
...
{
    uint8_t status = 0;
    LISA_LOGI(TAG, "app_ble_adv_start_handler:%d", adv_info->adv_type);
    ...
    {
        case BLE_ADV_GEN :
            LISA_LOGI(TAG, "ble adv general");
        case BLE_ADV_GEN_PAIRED :
        {
            LISA_LOGI(TAG, "ble adv general paired");
...
#if (ADV_USER_DATA)
            {
                /* 从可覆盖接口获取广播数据，sample 可自行实现 `lisa_bt_get_adv_data` 来定制 */
                uint8_t adv_len = 0;
                const uint8_t *adv_ptr = lisa_bt_get_adv_data(&adv_len);
                if (adv_ptr && adv_len) {
                    adv_param.adv_param.gen_adv.user_data_len = adv_len;
                    adv_param.adv_param.gen_adv.user_data = (uint8_t*)adv_ptr;
                } else {
                    uint8_t adv_user_data[LEGA_ADV_DATA_LEN];
                            /* fallback: 无实现或返回空则使用组件内置默认数据 */
...
#endif
            {
                /* 从可覆盖接口获取扫描响应数据，sample 可自行实现 `lisa_bt_get_scan_rsp_data` 来定制 */
                uint8_t rsp_len = 0;
                const uint8_t *rsp_ptr = lisa_bt_get_scan_rsp_data(&rsp_len);
                if (rsp_ptr && rsp_len) {
                    adv_param.adv_param.gen_adv.rsp_data_len = rsp_len;
                    adv_param.adv_param.gen_adv.rsp_data = (uint8_t*)rsp_ptr;
                } else {
                    /* fallback: 无实现或返回空则使用组件内置默认数据 */
        ...
        {
            LISA_LOGI(TAG, "ble adv hdc direct");
            uint16_t len = 6;
            if(bt_stack_nvs_get(NVS_ID_PEER_ADDRESS, (uint8_t*)&len, adv_param.adv_param.dir_adv.peer_addr) == NVDS_OK) 
            {
                adv_param.disc_mode = GAPM_ADV_MODE_NON_DISC;
