#include "button_adc.h"

static void adc_button_callback(button_event_t evt, button_adc_config_t *user_data)
{
    printf("adc_button_index: %d, event: %d\n", user_data->button_index, evt);
}

void test_adc_button(void)
{
    button_adc_config_t btn_adc_cfg = {
        .unit_id = 0,
        .adc_channel = 0,       // PB2
    };

    button_config_t btn_cfg = {
        .short_press_time_ms = 1500,
        .long_press_time_ms = 2000,
        .long_hold_time_ms = 4500,
        .press_logic_level = 1,
    };

    const uint16_t vol[10] = {369, 474, 602, 752, 913, 1051, 1203, 1340, 1494, 1641};

    for (int i = 0; i < CONFIG_ADC_BUTTON_MAX_BUTTON_PER_CHANNEL; i++) {
        btn_adc_cfg.button_index = i;

        if (i == 0) {
            btn_adc_cfg.min = (0 + vol[i]) / 2;
        } else {
            btn_adc_cfg.min = (vol[i - 1] + vol[i]) / 2;
        }

        if (i == 9) {
            btn_adc_cfg.max = (vol[i] + 3000) / 2;
        } else {
            btn_adc_cfg.max = (vol[i] + vol[i + 1]) / 2;
        }

        button_new_adc_device(&btn_cfg, &btn_adc_cfg, adc_button_callback);
    }

    // while (1) {
    //     lisa_thread_mdelay(1000);
    // }
}
