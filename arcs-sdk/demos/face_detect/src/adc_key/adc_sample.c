...
#define ADC_PIN        6       // GPIOB_06 -> Channel 4
#define ADC_DEVICE     "adc0"

static lisa_device_t *adc_dev = NULL;
...
int adc_sample_init()
{
    ...
    /* 获取ADC设备 */
    adc_dev = lisa_device_get(ADC_DEVICE);
...
int adc_sample_get_channel_value(uint32_t channel)
{
    ...
    uint32_t voltage = 0;
    int ret = lisa_adc_read(adc_dev, ADC_CHANNEL, &raw_value);

    if (ret == 0) {
        /* 转换为电压值 (mV) */
        voltage = LISA_ADC_RAW_TO_MV(raw_value, 3600, 10);
        LISA_LOGD(LOG_TAG, "Channel %d: raw=0x%04x (%4u), voltage=%4lu mV\n",
