...
// #define LSF_IC_ALLOCATOR_MEMSIZE 32bytes*N = 32/sizeof(MPOOLQ_CELL_T)个cells*N
// #define LSF_IC_ALLOCATOR_MEMSIZE (32*100)
__attribute__((aligned(32)))
uint8_t poolBuf_0[LSF_IC_ALLOCATOR_MEMSIZE];
...
// #define LSF_IC_ALLOCATOR_MEMSIZE (32*100)
__attribute__((aligned(32)))
uint8_t poolBuf_1[LSF_IC_ALLOCATOR_MEMSIZE];
...
// block memory pool - LsfPropertyParcel
XosBlockPool lsf_parcelMemPool;

__attribute__((aligned(32)))
static uint8_t lsf_parcelMemPool_buf[LSF_PARCEL_MEMPOOL_CAPACITY * IC_ROUNDUP_4(sizeof(LsfPropertyParcel))];

// block memory pool - LsfVariant
XosBlockPool lsf_variantMemPool;

__attribute__((aligned(32)))
static uint8_t lsf_variantMemPool_buf[LSF_VARIANT_MEMPOOL_CAPACITY * IC_ROUNDUP_4(sizeof(LsfVariant))];
...
int lsf_init(void)
{
    ...
    ret = xos_block_pool_init(&lsf_parcelMemPool, lsf_parcelMemPool_buf,
                              IC_ROUNDUP_4(sizeof(LsfPropertyParcel)),
                              LSF_PARCEL_MEMPOOL_CAPACITY,
                              XOS_BLOCKMEM_WAIT_PRIORITY);
    IC_ASSERT(0 == ret);

    ret = xos_block_pool_init(&lsf_variantMemPool, lsf_variantMemPool_buf,
                              IC_ROUNDUP_4(sizeof(LsfVariant)),
                              LSF_VARIANT_MEMPOOL_CAPACITY,
                              XOS_BLOCKMEM_WAIT_PRIORITY);
    IC_ASSERT(0 == ret);
...
int lsf_defaultMsgProc(LsfMsg *msg)
{
    ...
    if (LSF_MSG_PROPERTY_DISPATCH != msg->id) {
        CLOGD("[ScanService > LSF_MSG_PROPERTY_DISPATCH] skip msg->id: %d\r\n",
              msg->id);

        return -1;
    }

    CLOGD("[ScanService > LSF_MSG_PROPERTY_DISPATCH] msg->id: %d\r\n",
          msg->id);
    ...
    // 处理完, free(parcel)
    int32_t ret = xos_block_free(&lsf_parcelMemPool, parcel);
    IC_ASSERT(0 == ret);
