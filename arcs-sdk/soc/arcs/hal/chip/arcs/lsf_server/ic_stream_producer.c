              ...
              void *pcmBuffer, int pcmBufferLen, int mode)
{
    ...
    if (ICSTREAM_MODE_CONSUMER == mode) {
        ...
        // 数据帧数 信号量, 初始化为0
        self->sema_consumerDataFrames = xSemaphoreCreateCounting(share->frameCount_InternalBuf,
                                                                 0);
        IC_ASSERT(NULL != self->sema_consumerDataFrames);

        // 远程同步信号量, 初始化为0
        self->sema_consumerSyncPoint = xSemaphoreCreateCounting(1, 0);
        IC_ASSERT(NULL != self->sema_consumerSyncPoint);
    }
    else if (ICSTREAM_MODE_PRODUCER == mode) {
        // 空帧数信号量, 初始化为最大空帧数
        self->sema_producerEmptyFrames = xSemaphoreCreateCounting(share->frameCount_InternalBuf,
                                                                  share->frameCount_InternalBuf);
        IC_ASSERT(NULL != self->sema_producerEmptyFrames);

        // 远程同步信号量, 初始化为0
        self->sema_producerSyncPoint = xSemaphoreCreateCounting(1, 0);
        IC_ASSERT(NULL != self->sema_producerSyncPoint);
    }
...
int ICStream_detach(ICStream *self)
{
    ...
    if (ICSTREAM_MODE_PRODUCER == self->mode) {
        ...
        do {
            ret = xSemaphoreGive(self->sema_producerEmptyFrames);
        } while (ret == pdPASS);
    }
        ...
        CLOGD("[ICStream_detach]: sem = %d",
              uxSemaphoreGetCount(self->sema_consumerDataFrames));
        // 控制区不需要invalidate
        ...
        // 可能有异步并发+1. 不确定 frameCount_InternalBuf 次.
        do {
            ret = xSemaphoreTake(self->sema_consumerDataFrames, 0);
        } while (ret == pdPASS);
    }
...
ICStream_Producer_waitFrame(ICStream *self)
{
    ...
    // 等待"空帧数量" > 0
    ret = xSemaphoreTake(self->sema_producerEmptyFrames, portMAX_DELAY);
    IC_ASSERT(pdPASS == ret);

    xos_log_write(Producer_waitFrame_Take,
             (uint32_t) sem,
             (uint32_t) uxSemaphoreGetCount(sem));
...
ICStream_Producer_releaseFrame(ICStream *self, void *in_frame)
{
    ...
    ret = xSemaphoreTake(self->sema_producerEmptyFrames, 0);
...
_Producer_emptyFramesIncrement_handler(urpc_frame *frame)
{
    ...
    BaseType_t ret;
    // 唤醒"空帧数量信号量"
    ret = xSemaphoreGive(_ICStream_objs[objID]->sema_producerEmptyFrames);
    IC_ASSERT(pdPASS == ret);

    SemaphoreHandle_t sem = _ICStream_objs[objID]->sema_producerEmptyFrames;
    xos_log_write(EmptyFramesIncrement_push_responser_Give,
                  (uint32_t) emptyFrame,
                  (uint32_t) (uxSemaphoreGetCount(sem) << 16 | frame->header.session));
...
_Producer_dataFramesIncrement_registerPush_handler(urpc_frame* frame)
{
    ...
    // 以此为对象级的核间同步标志.
    ret = xSemaphoreGive(_ICStream_objs[objID]->sema_producerSyncPoint);
    IC_ASSERT(pdPASS == ret);
}
...
ICStream_Producer_syncWithConsumer(ICStream *self)
{
    ...
    // 以此为对象级的核间同步标志.
    ret = xSemaphoreTake(self->sema_producerSyncPoint, portMAX_DELAY);
    IC_ASSERT(pdPASS == ret);
...
{
    BaseType_t ret;
    SemaphoreHandle_t sem = self->sema_consumerDataFrames;

    // 等待"数据帧数量" > 0
    ret = xSemaphoreTake(self->sema_consumerDataFrames, portMAX_DELAY);
    IC_ASSERT(pdPASS == ret);
    ...
    ret = xSemaphoreGive(self->sema_consumerDataFrames);
    // 异步用法下, 信号量与实际'数据帧数量'不一致. 无法超过上限.
    // IC_ASSERT(pdPASS == ret);
...
_Consumer_dataFramesIncrement_handler(urpc_frame * frame)
{
    ...
    BaseType_t ret;
    // 唤醒"数据帧数量"信号量
    ret = xSemaphoreGive(_ICStream_objs[objID]->sema_consumerDataFrames);
    // 非mutex, holder不同也通过.
    // 异步用法下, 信号量与实际'数据帧数量'不一致. 无法超过上限.
    // IC_ASSERT(pdPASS == ret);

    SemaphoreHandle_t sem = _ICStream_objs[objID]->sema_consumerDataFrames;
...
_Consumer_emptyFramesIncrement_registerPush_handler(urpc_frame* frame)
{
    ...
    // 以此为对象级的核间同步标志.
    ret = xSemaphoreGive(_ICStream_objs[objID]->sema_consumerSyncPoint);
    IC_ASSERT(pdPASS == ret);
}
...
ICStream_Consumer_syncWithProducer(ICStream *self)
{
    ...
    // 以此为对象级的核间同步标志.
    ret = xSemaphoreTake(self->sema_consumerSyncPoint, portMAX_DELAY);
    IC_ASSERT(pdPASS == ret);
