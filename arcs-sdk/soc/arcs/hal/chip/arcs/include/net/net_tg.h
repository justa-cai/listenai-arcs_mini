 ...
 * @file net_tg.h
 *
 * @brief Definitions of  TG (traffic generator) task.
 *
...
#define NET_TG_MAX_TRAFFIC_STREAMS 4
/// Number of TG send buffers (credits)
#define NET_TG_SEND_BUF_CNT        MAC_TXQ_DEPTH

/// Traffic sending direction
#define NET_TG_DIRECT_SEND                1
/// Traffic receiving direction
#define NET_TG_DIRECT_RECV                2
...
enum profile_id
{
    ...
    NET_PROF_START_SYNC,
    /// NOT USED
    NET_PROF_CALI_RTD = NET_PROF_START_SYNC,
    /// NOT USED
    NET_PROF_UAPSD,
    /// NOT USED
    NET_PROF_LAST,
...
/// TG stream
struct net_tg_stream
{
    /// Stream ID
    uint32_t id;
    /// State of TG stream (true for active, false for inactive)
    bool active;
    /// TG semaphore used to synchronize the TG thread and the FHOST CLI one
    ...
    struct net_tg_time transc_timestamp;
    /// TG profile
    struct net_tg_profile prof;
    /// TG statistics
    struct net_tg_stats stats;
};
 ...
 */
/// Table of TG streams
extern struct net_tg_stream g_streams[NET_TG_MAX_TRAFFIC_STREAMS];
 ...
 ****************************************************************************************
 */
int net_tg_config(uint32_t stream_id, uint32_t prof, int direction, uint32_t rip,
                    uint32_t remote_port, uint32_t lip, uint32_t local_port,
                    uint32_t rate, uint32_t pksize, uint32_t duration, uint16_t tos);
 ...
 ****************************************************************************************
 */
int net_tg_start(uint32_t stream_id);
 ...
 ****************************************************************************************
 */
struct net_tg_stats *net_tg_stop(uint32_t stream_id);
