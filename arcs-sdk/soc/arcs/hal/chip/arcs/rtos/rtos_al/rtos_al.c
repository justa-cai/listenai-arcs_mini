...
rtos_stack_type cntrl_task_stack_buf[512];
rtos_static_task_tcb cntrl_task_control;
rtos_stack_type fhost_rx_task_stack_buf[512];
rtos_static_task_tcb fhost_rx_task_control;
rtos_stack_type fhost_tx_task_stack_buf[384];
rtos_static_task_tcb fhost_tx_task_control;
rtos_stack_type wpa_task_stack_buf[1024];
rtos_static_task_tcb wpa_task_control;
rtos_stack_type wifi_task_stack_buf[768];
rtos_static_task_tcb wifi_task_control;
...
void *rtos_aligned_malloc(uint32_t size, uint32_t alignment)
{
    ...
    ptr = pvPortMalloc(aligned_size + alignment - 1 + sizeof(void *));
    if (ptr == NULL)
    {
        return NULL;
    }

    aligned_ptr = (void *)(((uint32_t)ptr + sizeof(void *) + alignment - 1) & ~(alignment - 1));
...
{
    return xTimerCreate(NULL, pdMS_TO_TICKS(period_ms), reload, id, cb);
}
...
void rtos_get_cpu_usage( char * pcWriteBuffer, int32_t uxBufferLength )
{
    ...
    configRUN_TIME_COUNTER_TYPE *task_time_stats;
