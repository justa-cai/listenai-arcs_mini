...
int LsfPropertyService_addGroup(int group_id, LsfPropertyConfig *config_array, int config_count)
{
    ...
    // 预备 管理表
    LsfPropertyItem *property_items = (LsfPropertyItem *) pvPortMalloc(config_count * sizeof(LsfPropertyItem));
...
int LsfPropertyService_dispatch(LsfPropertyItem *property_item, LsfPropertyParcel *parcel)
{
    ...
    if (LSF_PROPERTY_CONTEXT_TASK == property_item->context_type) {
        ...
        ret = xQueueSendToBack(property_item->context_msgQ, & msg, 0);
...
static void _LsfPropertyService_Notify_push_responser(urpc_frame * frame)
{
    ...
    // 分包
    LsfPropertyParcel *parcel = (LsfPropertyParcel *) pvPortMalloc(sizeof(LsfPropertyParcel));
...
// lazy init and get thread local storage msgQ for set/get sync
static inline
QueueHandle_t LsfProperty_getTLSmsgQ(void)
{
    void* sync_msgQ;

    // Read the value stored in index of the calling task's thread local storage array
    sync_msgQ = pvTaskGetThreadLocalStoragePointer(NULL, LSF_PROPERTY_TLS_INDEX);

    if (sync_msgQ == NULL) {
        // set/get同步用的msgQ, thread唯一
        // TODO: 销毁放哪里?
        sync_msgQ = xQueueCreate(1 , sizeof(LsfMsg));
        IC_ASSERT(NULL != sync_msgQ);

        // memory barrier: write-release. 暂用: full system, any-any = 15
        __DSB();

        // Store the msgQ to index of the calling task's thread local storage array
        vTaskSetThreadLocalStoragePointer(NULL, LSF_PROPERTY_TLS_INDEX, (void*) sync_msgQ);
    }
...
LsfProperty* LsfProperty_new(int type)
{
    ...
    LsfProperty *self = (LsfProperty *) pvPortMalloc(sizeof(LsfProperty));
    IC_ASSERT(self != NULL);
...
int LsfProperty_set(LsfProperty *self, int group_id, int property_id, LsfVariant *var)
{
    ...
    QueueHandle_t sync_msgQ = LsfProperty_getTLSmsgQ();
    ...
    else if (var->variant_type == type_membuf) {
    ...
    if ((property_item->flag & LSF_PROPERTY_FLAG_SYNCMODE_MSK) == LSF_PROPERTY_FLAG_SYNC) {
        ...
        BaseType_t result;
        LsfMsg msg;
        result = xQueueReceive(sync_msgQ, & msg, portMAX_DELAY);
        IC_ASSERT(pdPASS == result);

        // 必然是
        if (LSF_MSG_PROPERTY_SET_RESPONSE != msg.id) {
            IC_ASSERT(false);
        }

        // 返回值: uint类型
        LsfVariant *out_var = (LsfVariant *) msg.param;
...
_LsfPropertyService_Set_Async_Responser(urpc_frame * frame)
{
    ...
    QueueHandle_t sync_msgQ = (QueueHandle_t) read_u32((uint8_t *)(& (frame->rpc.payload[0])));
    ...
    LsfVariant *new_var = (LsfVariant *) pvPortMalloc(sizeof(LsfVariant));
    ...
    // LSF_PROPERTY_FLAG_SYNC: 等待中, 通知解除阻塞
    if ((property_item->flag & LSF_PROPERTY_FLAG_SYNCMODE_MSK) == LSF_PROPERTY_FLAG_SYNC) {
        LsfMsg msg = {
            .id = LSF_MSG_PROPERTY_SET_RESPONSE,
            .param = (uint32_t) new_var,
        };
        BaseType_t os_ret = xQueueSendToBack(sync_msgQ, & msg , 0);
        IC_ASSERT(pdPASS == os_ret);
    }
