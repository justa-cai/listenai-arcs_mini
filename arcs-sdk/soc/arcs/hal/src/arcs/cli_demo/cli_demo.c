...
#if CFG_NVS
#define NVDS_FLASH_ADDRESS   (CMN_FLASH_REGION +  0x201000) //offset 1152KB
...
#endif
#if CONFIG_PM
    /*Since UART0 is used in the AP’s ROM boot,
    *the CP can only use UART0 as well to avoid conflicts.*/
    logInit(SHELL_UART1, SHELL_UART0_BAUDRATE);
#else
