...
void yyuv_to_yvyu(uint32_t *image_buf, uint32_t size_word)
{
    ...
    {
        *image_buf = (*image_buf & 0xFF000000) | ((*image_buf & 0xFF0000) >> 8) | ((*image_buf & 0xFF00) << 8) | (*image_buf & 0xFF);
        image_buf++;
    }
}
...
void yuv444_to_rgb888(uint8_t *Ydata, uint8_t *Udata, uint8_t *Vdata, uint8_t *RGBdata, uint32_t pixel)
{
    ...
    while(pixel--)
    {
        ...
        *RGBdata++ = CONSTRAIN_UINT8(b);
        *RGBdata++ = CONSTRAIN_UINT8(g);
        *RGBdata++ = CONSTRAIN_UINT8(r);
    }
}
...
#define TUNE(x) ( x < 0 ? 0 : (x > 255 ? 255 : x) )

#define GetY(R, G, B)   ((263 *(R) + 516 *(G) + 100 *(B) + 16384) >> 10)
#define GetU(R, G, B)   ((-151 *(R) - 298 *(G) + 450 *(B) + 131072) >> 10)
#define GetV(R, G, B)   ((450 *(R) - 377 *(G) - 73 *(B) + 131072) >> 10)
#define GetR(Y, U, V)   ((1192 *((Y) - 16) + 1634 *((V) - 128)) >> 10)
#define GetG(Y, U, V)   ((1192 *((Y) - 16) - 832 *((V) - 128) - 401 *((U) - 128)) >> 10)
#define GetB(Y, U, V)   ((1192 *((Y) - 16) + 2066 *((U) - 128)) >> 10)
...
void rgb565_to_yuv422yuyv(uint16_t *rgb565, uint8_t *yuv422, uint32_t pixel)
{
    ...
    {
        rgb565_value = *rgb565++;
        b = PIXEL_RGB565_B5(rgb565_value) << 3;
        g = PIXEL_RGB565_G6(rgb565_value) << 2;
        r = PIXEL_RGB565_R5(rgb565_value) << 3;
...
void yuv444yuv_create(uint8_t *yuv444, uint32_t pixel, uint32_t rgb888_color)
{
    ...
    r = (rgb888_color >> 16) & 0xFF;
    g = (rgb888_color >> 8) & 0xFF;
    b = rgb888_color & 0xFF;
...
void yuv444_to_rgb888(uint8_t *Ydata, uint8_t *Udata, uint8_t *Vdata, uint8_t *RGBdata, uint32_t pixel)
{
    ...
    while(pixel--)
    {
        ...
        *RGBdata++ = CONSTRAIN_UINT8(bi);
        *RGBdata++ = CONSTRAIN_UINT8(gi);
        *RGBdata++ = CONSTRAIN_UINT8(ri);
    }
}
...
void yuv422_to_rgb565(uint8_t *Ydata, uint8_t *Udata, uint8_t *Vdata, uint8_t *RGBdata, uint32_t pixel)
{
    ...
    while(pixel--)
    {
        ...
        r = CONSTRAIN_UINT8(ri);
        g = CONSTRAIN_UINT8(gi);
        b = CONSTRAIN_UINT8(bi);
