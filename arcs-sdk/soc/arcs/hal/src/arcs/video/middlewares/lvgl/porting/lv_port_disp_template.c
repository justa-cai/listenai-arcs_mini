...
#define LCD_INIT(p)    										lcd_init(p)
#define LCD_DISPLAY(Xstart, Ystart, Xend, Yend, pImage)    	lcd_display(Xstart, Ystart, Xend, Yend, pImage)
...
#else
    static lv_color_t buf_1[MY_DISP_HOR_RES * MY_DISP_VER_RES];                                              /* 设置缓冲区的大小为 10 行屏幕的大小 */
    lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_1, NULL, MY_DISP_HOR_RES * MY_DISP_VER_RES);                  /* 初始化显示缓冲区 */
#endif
...
{
    /*You code here*/
	LCD_INIT(NULL);         /* 初始化LCD */
    //lcd_display_dir(1); /* 设置横屏 */
}

#define SWAP_UINT16(x)  (((x & 0x00FF) << 8) | ((x & 0xFF00) >> 8))

static void swapuint16(uint16_t *pdata, uint32_t num)
{
	while(num--)
	{
		*pdata = SWAP_UINT16(*pdata);
		pdata++;
	}
}
...
//    lcd_color_fill(area->x1, area->y1, area->x2, area->y2, (uint16_t *)color_p);
    ...
    LCD_DISPLAY(area->x1, area->y1, area->x2, area->y2, color_p);
