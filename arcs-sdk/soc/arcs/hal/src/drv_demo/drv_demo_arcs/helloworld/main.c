...
int main(void)
{
    ...
    CLOGD("[%s:%d] main addr is 0x%x", __func__, __LINE__, &main);
    CLOGD("[%s:%d] RAM addr is 0x%x", __func__, __LINE__, &times);
    ...
    CLOGD("Cluster %d, Hart %d, MISA: 0x%lx\r\n", clusterid, hartid, misa);
    print_misa();
    ...
    srand(seed);
    rval = rand();
    CLOGD("Got rand integer %d using seed %d.\r\n", seed, rval);

    for (unsigned long i = 0; i < RUN_LOOPS; i ++) {
        CLOGD("%d: Hello World From Nuclei RISC-V Processor!\r\n", i);
    }

    while(1)
    {
        SysTick_Delay_Ms(1000);
        CLOGD("[%s:%d] times=%d", __func__, __LINE__, times++);
    }
