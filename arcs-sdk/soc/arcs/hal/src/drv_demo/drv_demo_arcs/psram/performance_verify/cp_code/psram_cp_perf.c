...
// SRAM
#define PERFORMANCE_VERIFY_SRAM_MEMORY                 (uint32_t*)CMN_RAM1_REGION
#define PERFORMANCE_VERIFY_SRAM_SIZE                   (uint32_t)(CMN_RAM1_REGION_SIZE)
// SRAM DMA
#define PERFORMANCE_VERIFY_SRAM_MEMORY_DMA             (uint32_t*)CMN_RAM1_REGION
#define PERFORMANCE_VERIFY_SRAM_SIZE_DMA               (uint32_t)(CMN_RAM1_REGION_SIZE)
...
static void PSRAM_PerformanceVerify_FlashMemory2PSRAM(){
    ...
    SysTick_Close();
    {
        ...
        CLOGD("[Performance] Flash to PSRAM speed -> %d B/s", (uint32_t)t_speed);
    }
}
