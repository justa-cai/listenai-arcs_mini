...
static void pmp_violation_fault_handler(unsigned long mcause, unsigned long sp)
{
    ...
    switch (mcause & MCAUSE_CAUSE) {
        case InsAccFault_EXCn:
            printf("Instruction access fault occurs, cause: 0x%lx, mdcause: 0x%lx, epc: 0x%lx\r\n", exc_frame->cause, __RV_CSR_READ(CSR_MDCAUSE), exc_frame->epc);
            break;
        case LdFault_EXCn:
            printf("Load access fault occurs, cause: 0x%lx, mdcause: 0x%lx, epc: 0x%lx\r\n", exc_frame->cause, __RV_CSR_READ(CSR_MDCAUSE), exc_frame->epc);
            break;
        case StAccessFault_EXCn:
            printf("Store/AMO access fault occurs, cause: 0x%lx, mdcause: 0x%lx, epc: 0x%lx\r\n", exc_frame->cause, __RV_CSR_READ(CSR_MDCAUSE), exc_frame->epc);
            break;
        default: break;
    }
    while(1);
