...
int main()
{
	int iocfg = 1;
	volatile int rdata;
	unsigned int ref_data_l[80];
	j=0;
	intr_apc_done = 0;
	CLOGD("APC I2S0 MST TXRX 32bit Stero test\n");
	srand(100);
	for(int i=0;i<80;i++){
	...
	for(int i=0; i<80; i++) {
		ref_data_l[i] = (i%2) ? (0xffffffff-(i/2)):(0xaaaaaaaa+(i/2));
	}
	...
	IP_AUDIO_APC->REG_APC_TX_CH0_CFG.bit.TX_CH0_L_MODE = 0x2;
	//TX Channel R Mode 24bit
	IP_AUDIO_APC->REG_APC_TX_CH0_CFG.bit.TX_CH0_R_MODE = 0x2;
	//RX Channel L Mode 24bit
	IP_AUDIO_APC->REG_APC_RX_CH0_CFG.bit.RX_CH0_L_MODE = 0x2;
	//RX Channel R Mode 24bit
	IP_AUDIO_APC->REG_APC_RX_CH0_CFG.bit.RX_CH0_R_MODE = 0x2;
	...
	//
	IP_AUDIO_APC->REG_APC_I2S0_CFG0.bit.I2S0_BCK_LRCK = 0x7;
	//Right justified
	IP_AUDIO_APC->REG_APC_I2S0_CFG0.bit.I2S0_RIGHT_JUSTIFIED = 0x1;
	...
	do{
		rdata = IP_AUDIO_APC->REG_APC_TX_CH0_CFG.bit.TX_CH0_L_FIFO_CNT;
	}while(!rdata);
