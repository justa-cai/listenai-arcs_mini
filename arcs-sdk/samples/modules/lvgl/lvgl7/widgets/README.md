...
演示 LVGL 7.x 图形库的 Widgets 组件功能，展示各种 UI 控件的使用方法和视觉效果。

本示例使用 LISA Display 和 LISA Touch 设备框架，通过 SPI 接口驱动 ST7789P3 LCD 屏幕，通过 I2C 接口驱动 CST328 电容触摸屏，实现完整的图形界面交互。
...
**屏幕显示：**
- 显示 LVGL widgets 演示界面
- 包含按钮、滑块、开关、下拉框等各种 UI 控件
- 支持触摸交互
...
{
    lv_demo_widgets();
    LISA_LOGI(LOG_TAG, "LVGL widgets start");
...
5. **触摸坐标转换**：LVGL 移植层会自动根据屏幕旋转角度转换触摸坐标