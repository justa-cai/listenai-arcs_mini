...
/**
 * @file main.c
 * @brief LISA QSPILCD PIO 传输示例
 *
 * 演示使用 QSPI LCD 驱动进行 PIO 模式的数据传输
 */
...
int main(int argc, char **argv)
{
    ...
    LISA_LOGI(LOG_TAG, "=== LISA QSPILCD PIO example ===");
    ...
    /* ========== 示例 2: PIO 模式传输（四线） ========== */
    {
        ...
        if (ret == LISA_DEVICE_OK) {
            LISA_LOGI(LOG_TAG, "PIO transfer (Quad lane, 8-bit): OK");
        } else {
            LISA_LOGE(LOG_TAG, "PIO transfer failed: %d", ret);
        }
    }
