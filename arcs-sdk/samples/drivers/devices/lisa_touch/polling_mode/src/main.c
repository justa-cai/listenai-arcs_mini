...
/**
 * @file main.c
 * @brief LISA Touch 轮询模式示例
 *
 * 本示例演示如何使用 LISA Touch 驱动的轮询模式：
 * 1. 初始化Touch设备
 * 2. 配置I2C和GPIO总线
 * 3. 设置轮询模式（默认模式）
 * 4. 循环读取触摸事件并显示坐标
...
{
    printf("=== LISA Touch Polling Mode Example ===\n");
    ...
    /* 设置轮询模式（可选，默认就是轮询模式） */
    ret = lisa_touch_set_int_mode(touch_dev, LISA_TOUCH_INT_MODE_POLLING);
    if (ret != 0) {
        printf("Error: Touch polling mode set failed (code: %d)\n", ret);
        return -1;
    }
    printf("Touch polling mode enabled\n");
    ...
    printf("\n=== Polling Mode Active ===\n");
    printf("Start reading touch events (polling every 50ms)...\n");
    printf("Press the touch screen to see coordinates\n\n");

    /* 循环读取触摸事件 */
    uint32_t error_count = 0;   /* 错误计数器 */
    
    while (1) {
        lisa_touch_event_t event;
        ret = lisa_touch_read_event(touch_dev, &event);
        ...
        if (ret == 0) {
            ...
            error_count++;
            if (error_count <= 3) {
                printf("Warning: Touch read failed (code: %d, count: %u)\n", ret, error_count);
            } else if (error_count == 4) {
                printf("Warning: Too many errors, suppressing further messages\n");
            }
        }
