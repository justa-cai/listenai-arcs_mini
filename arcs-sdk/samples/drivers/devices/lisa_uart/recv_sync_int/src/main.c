...
/**
 * @file main.c
 * @brief LISA UART 同步接收示例（中断模式 + 循环缓冲区）
 *
...
{
    LISA_LOGI(LOG_TAG, "=== LISA UART Recv Sync (Interrupt Mode + Circular Buffer) ===");
    ...
    LISA_LOGI(LOG_TAG, "  - Interrupt mode with idle interrupt");
