...
/**
 * @file main.c
 * @brief LISA UART 中断模式异步发送示例
 *
 * 演示在中断模式下使用 lisa_uart_write_async() 异步发送数据，
 * 通过事件回调接收发送完成通知
 */
...
{
    LISA_LOGI(LOG_TAG, "=== LISA UART async send (interrupt mode) example ===");
    ...
    /* 3. 配置 UART (115200, 8N1, INTERRUPT) */
