...
/// Maximum ping time for sending process (4000 s)
#define NET_TG_MAX_PING_TIME       4000000000
...
net_tg_send_buf_mem[NET_TG_MAX_TRAFFIC_STREAMS][NET_TG_SEND_BUF_CNT] __SHAREDRAM;
#endif // NX_TG
                           ...
                           u32_t *sleep_time, u32_t *throttled_rate)
{
    ...
    *sleep_time = 0;
    ...
    switch(prof)
    {
        ...
        break;
        #endif // NX_TG
        // Ping command profile @ref net_ping.h
        case NET_PROF_PING :
        throttled_rate_int = 1;
        if (rate > 50)
        {
 ...
 ****************************************************************************************
 **/
static int net_time_diff(struct net_tg_time *t1, struct net_tg_time *t2)
{
    int dtime;
...
static void net_tg_pbuf_custom_payload_free_cb(void *p)
{
    ...
    struct net_tg_stream *stream = buf->info.stream;
 ...
 ****************************************************************************************
 **/
static void net_tg_send_buf_init(struct net_tg_stream *stream)
{
    int i, j;
    ...
    memset(net_tg_send_buf_mem[i], 0, sizeof(struct net_tg_send_buf_tag)
            * NET_TG_SEND_BUF_CNT);
    for (j = 0; j < NET_TG_SEND_BUF_CNT; j++)
    {
 ...
 ****************************************************************************************
 **/
static struct net_tg_send_buf_tag *net_tg_find_free_send_buf(struct net_tg_stream *stream)
{
    int i, j;
 ...
 ****************************************************************************************
 **/
static int net_cal_throughput(struct net_tg_time *init, struct net_tg_time *actual,
                              u32_t tx_bytes)
{
    int diff_time;
    int throughput;
                           ...
                           const ip_addr_t *addr, u16_t port)
{
    if (p != NULL)
    {
        ...
        struct net_tg_stream *stream = arg;
                                  ...
                                  const ip_addr_t *addr, u16_t port)
{
    ...
    tot_len = p->tot_len;
    pbuf_free(p);
    rtos_get_sys_time(SINCE_EPOCH, &actual.sec, &actual.usec);
                                  ...
                                  const ip_addr_t *addr, u16_t port)
{
    struct net_tg_stream *stream = arg;
    int diff_time;
    struct net_tg_time recv;

    if (p == NULL)
        return;

    rtos_get_sys_time(SINCE_EPOCH, &recv.sec, &recv.usec);
 ...
 ****************************************************************************************
 **/
static net_if_t *net_tg_get_netif(struct net_tg_stream *stream)
{
 ...
 ****************************************************************************************
 **/
static int net_tg_mcast_membership(struct net_tg_stream *stream, bool join)
{
    #if LWIP_IGMP
    ip_addr_t grp_ip;
 ...
 ****************************************************************************************
 **/
static int net_tg_pcb_config(struct udp_pcb *pcb, struct net_tg_stream *stream)
{
    u32_t ip;
    ip_addr_t rip;
    ip_addr_t lip;
    ...
    if(stream->prof.prof_id == NET_PROF_MCAST)
    {
        if (stream->prof.direction == NET_TG_DIRECT_SEND)
 ...
 ****************************************************************************************
 **/
static struct udp_pcb *net_tg_init(struct net_tg_stream *stream, udp_recv_fn recv)
{
 ...
 ****************************************************************************************
 **/
static int net_ping_pcb_config(struct raw_pcb *pcb, struct net_ping_stream *stream)
{
    u32_t ip;
    ip_addr_t rip;
 ...
 ****************************************************************************************
 **/
static struct raw_pcb *net_ping_init(struct net_ping_stream *stream,
...
{
	struct net_ping_stream *stream = arg;
	struct icmp_echo_hdr *iecho;
	size_t hdr_len = NET_TG_PING_HDR_LEN;
	char *sec_p, *usec_p;
	int diff_time;
	struct net_tg_time send;
	...
	if (pbuf_header(p, -PBUF_IP_HLEN) == 0) {
		...
		if ((iecho->id == NET_TG_PING_ID + stream->id) &&
			...
			rtos_get_sys_time(SINCE_EPOCH, &stream->stats.last_msg.sec,
			...
			CLOG("%d bytes from: %s: icmp_req=%d ttl=%d time=%d us, echo %x\n",
				p->tot_len, ipaddr_ntoa(&pcb->remote_ip),
				reply_num, pcb->ttl, diff_time, iecho->type);
                                  ...
                                  struct net_ping_stream *stream)
{
    ...
    rtos_get_sys_time(SINCE_EPOCH, &stream->stats.last_msg.sec, &stream->stats.last_msg.usec);
...
#if NX_TG
int net_tg_send_transc(struct net_tg_stream *stream)
{
    struct udp_pcb * pcb = NULL;
    net_buf_tx_t *h = NULL;
    net_buf_tx_t *p = NULL;
    err_t err;
    struct net_tg_time send, init;
    ...
    rtos_get_sys_time(SINCE_EPOCH, &init.sec, &init.usec);
    while (stream->active && duration_cur < duration)
    {
        ...
        rtos_get_sys_time(SINCE_EPOCH, &send.sec, &send.usec);
...
int net_tg_send(struct net_tg_stream *stream)
{
    struct udp_pcb * pcb = NULL;
    net_buf_tx_t *h = NULL;
    net_buf_tx_t *p = NULL;
    ...
    int status = 0;
    err_t err;
    struct net_tg_time before, after, actual, init;
    int diff_time;
    u32_t sleep_time = 0;
    ...
    while (stream->active && duration_cur < duration)
    {
        ...
        {
            rtos_get_sys_time(SINCE_EPOCH, &before.sec, &before.usec);
        ...
        if (throttled_rate != 0)
        {
            ...
            {
                rtos_get_sys_time(SINCE_EPOCH, &after.sec, &after.usec);
    ...
    while (stream->credits < NET_TG_SEND_BUF_CNT)
    {
        rtos_semaphore_wait(stream->net_tg_semaphore, -1);
    }
...
int net_tg_recv_start(struct net_tg_stream *stream)
{
...
int net_tg_recv_transc_start(struct net_tg_stream *stream)
{
...
void net_tg_recv_stop(struct net_tg_stream *stream)
{
    LOCK_TCPIP_CORE();
...
int net_ping_send(struct net_ping_stream *stream)
{
    ...
    net_sleep_time(NET_PROF_PING, stream->prof.rate,
                   &sleep_time, NULL);

    while (stream->active && (continuous || (duration > 0)))
    {
