...
struct _ICStream {
    ...
    union {
        ...
        SemaphoreHandle_t sema_producerEmptyFrames;

        // 对象的单核consumer私有部分
        // 远程同步信号量: 数据帧数量. 本端consumer等待, 由对端通知唤醒.
        SemaphoreHandle_t sema_consumerDataFrames;
    };

    union {
        // 对象的单核producer私有部分
        // 远程同步信号量: 双端对象同步. 同步点后, 双端才开始运转.
        SemaphoreHandle_t sema_producerSyncPoint;

        // 对象的单核consumer私有部分
        // 远程同步信号量: 双端对象同步. 同步点后, 双端才开始运转.
        SemaphoreHandle_t sema_consumerSyncPoint;
    };
