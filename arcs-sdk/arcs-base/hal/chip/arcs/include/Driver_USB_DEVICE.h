...
#define USBD_DEVICE_DESCR_DEFINE(p) \
	__in_section(usb, descriptor_##p, 0) __used __aligned(1)
...
#define USBD_STRING_DESCR_DEFINE(p) \
	__in_section(usb, descriptor_##p, 4) __used __aligned(1)
#define USBD_TERM_DESCR_DEFINE(p) \
	__in_section(usb, descriptor_##p, 5) __used __aligned(1)
...
#define USBD_CFG_DATA_DEFINE(p, name) \
	__attribute__((aligned(1), section(".usb_data"))) __used

#define USBD_AUDIO_DATA_DEFINE(p) \
	__in_section(usb, p, 0) __used __aligned(1)
...
/**
 * @brief Configure endpoint dma callback function(endpoint with DMA mode)
 *
 * @param[in]  ep           Endpoint address corresponding to the one
 *                          listed in the device configuration table
 *
 * @return 0 on success, negative errno code on fail.
 */
int usb_ep_set_ep_dma_cb(const uint8_t ep,const usb_dc_ep_callback cb);


/**
 * @brief Configure endpoint callback function(endpoint without DMA mode)
 *
 * @param[in]  ep           Endpoint address corresponding to the one
 *                          listed in the device configuration table
 *
 * @return 0 on success, negative errno code on fail.
 */
int usb_ep_set_ep_cb(const uint8_t ep,const usb_dc_ep_callback cb);


/**
 * @brief Endpoint is configured with DMA enabled.
 *
 *
 * @param[in]  ep           Endpoint address corresponding to the one
 *                          listed in the device configuration table
 ...
 * @return 0 on success, negative errno code on fail.
 */
int usb_ep_enable_dma(const uint8_t ep);


/**
 * @brief Endpoint is configured with DMA diabled.
 *
 *
 * @param[in]  ep           Endpoint address corresponding to the one
 *                          listed in the device configuration table
 ...
 * @return 0 on success, negative errno code on fail.
 */
int usb_ep_disable_dma(const uint8_t ep);


/*
 * @brief Wait until USB enumerate done.
 */
void usb_enumerate_wait(void);
