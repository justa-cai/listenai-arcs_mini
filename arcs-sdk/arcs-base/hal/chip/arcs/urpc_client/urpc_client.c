...
#define URPC_CLIENT_STACK_SIZE  configMINIMAL_STACK_SIZE
...
static int8_t _urpc_wait_sync_client(uint32_t section_id, urpc_frame* frame, uint32_t timeout)
{
	if(xQueueReceive(sync_queue, frame, pdMS_TO_TICKS(timeout)) == pdPASS) { // receive a message
		return URPC_SUCCESS;
	} else {
		return URPC_TIMEOUT;
	}
...
int8_t urpc_trans_sync_client_with_timeout(urpc_client_stub* stub, urpc_frame* send, urpc_frame* recv, uint32_t timeout)
{
    ...
    if (err == URPC_SUCCESS) {
        err = _urpc_wait_sync_client(send->header.session, recv, timeout);
...
{
	return urpc_trans_sync_client_with_timeout(stub, send, recv, URPC_SYNC_TIMEOUT);
}
...
int8_t _urpc_recv_notify_client(urpc_client_stub* stub, urpc_connection* conn, uint8_t chn)
{
	...
	if(err) {
		if(rpc_cb->endpoints[chn].handles) {
			if(rpc_cb->endpoints[chn].handles[0].handler) {
				recv_frame.header.magic = err;
				(rpc_cb->endpoints[chn].handles[0].handler)(&recv_frame);
			} else {
				CLOGE("no endpoints handles error, <%d, %d, %x>\n", chn, err, recv_frame.header.flags);
			}
		} else {
			CLOGE("no endpoints error, <%d, %d, %x>\n", chn, err, recv_frame.header.flags);
		}
	}
	// receive next one
...
//	xTaskCreate(urpc_task_client, "rpc_client", 2048, NULL, 23, NULL);
	xTaskCreate(urpc_task_client, "rpc_client",
			URPC_CLIENT_STACK_SIZE, stub, configMAX_PRIORITIES - 2, NULL);
	urpc_recv((urpc_stub*)stub, URPC_CONN_CLIENT, &recv_frame);
