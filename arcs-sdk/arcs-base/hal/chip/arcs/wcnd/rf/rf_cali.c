...
/* Map power to IQ, power unit is mv */
#define RX_POW2IQ(target_power, full_power, bitwidth) (target_power * (1 << bitwidth) / full_power)
#define RX_POW2IQ_LEVEL(iq, target_power, full_power, bitwidth) (iq * full_power /(target_power * (1 << bitwidth)))
#define WF_MAX_SC_IDX 4
#define BT_MAX_SC_IDX 3
#define RX_CAL_IQ_BITWIDTH 12
//#define RX_CAL_WF_DCOC_PWR 130 //130mv this will be applied in new products to replace 400mv
#define RX_CAL_WF_DCOC_PWR 400 //400mv
#define RX_CAL_BT_DCOC_PWR 300 //300mv

static int rf_cali_rxdcoc_get_sc_idx(int base_step, int iq, int full_power, int max_idx)
{
    int threshold = 1;
    int level = RX_POW2IQ_LEVEL(iq, base_step, full_power, RX_CAL_IQ_BITWIDTH);
    int index = 0;
    ...
    return (index + 1) > max_idx ? max_idx : (index + 1);
}

__STATIC void rf_cali_rxdcoc_sel_sc(uint8_t mode, int32_t dc_i, int32_t dc_q)
{
    uint8_t sc_i = 1;
    uint8_t sc_q = 1;
    ...
    {
        sc_i = rf_cali_rxdcoc_get_sc_idx(RX_CAL_WF_DCOC_PWR, abs(dc_i), 1200, WF_MAX_SC_IDX);
        sc_q = rf_cali_rxdcoc_get_sc_idx(RX_CAL_WF_DCOC_PWR, abs(dc_q), 1200, WF_MAX_SC_IDX);
    }
    else
    {
        sc_i = rf_cali_rxdcoc_get_sc_idx(RX_CAL_BT_DCOC_PWR, abs(dc_i), 1200, BT_MAX_SC_IDX);
        sc_q = rf_cali_rxdcoc_get_sc_idx(RX_CAL_BT_DCOC_PWR, abs(dc_q), 1200, BT_MAX_SC_IDX);
    }
...
__STATIC int8_t rf_cali_rxiq_wf()
{
    ...
    if (rf_cali_check_iq_result(c21_l, c22_l, RX_IQ_C21_THD, RX_IQ_C22_THD)) {
        CLOGW("RXIQ abort since c21(%d) c22(%d) out of range\n", c21_l, c22_l);
        goto fail;
    }
#endif
...
__STATIC int8_t rf_cali_txiq(uint8_t fb_gain)
{
    ...
    calc_rxiq_matrix(&c21, &c22, Isq, Qsq, IxQ);
#if defined(CALI_CHECK_RESULT)
    if (rf_cali_check_iq_result(c21, c22, RX_IQ_C21_THD, RX_IQ_C22_THD)) {
        CLOGW("TX feedback path IQ abort since c21(%d) c22(%d) out of range\n", c21, c22);
        goto fail;
    }
#endif
...
__STATIC int8_t rf_cali_txdpd_one(uint8_t tbl_idx, int8_t pwr_idx, uint8_t iter_times, uint8_t fb_gain, complexint16 *mix_off_dc, uint8_t pwr_idx_dc, uint8_t pwr_idx_iq, complexint16* dpd_para_est)
{
    ...
    for (t = 0; t < iter_times; t++) {
        ...
        #if defined(CALI_CHECK_RESULT)
            if (t == iter_times - 1) {
                if (rf_cali_check_iq_result(c21, c22, TX_IQ_C21_THD, TX_IQ_C22_THD)) {
                    CLOGW("TXIQ abort since c21(%d) c22(%d) out of range\n", c21, c22);
                    c21 = 0;
                    c22 = 2048;
                }
            }
