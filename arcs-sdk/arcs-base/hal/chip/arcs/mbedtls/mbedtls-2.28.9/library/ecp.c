                                ...
                                mbedtls_ecp_restart_ctx *rs_ctx)
{
    ...
    if (rs_ctx == NULL)
    {
        int hsu_curve = HSU_EC_MAX, prime_limb;
        if (grp->id == MBEDTLS_ECP_DP_SECP256R1) {
            hsu_curve = HSU_EC_256;
            prime_limb = (32 + sizeof(mbedtls_mpi_uint) - 1) / sizeof(mbedtls_mpi_uint);
        } else if(grp->id <= MBEDTLS_ECP_DP_SECP384R1) {
            hsu_curve = HSU_EC_384;
            prime_limb = (48 + sizeof(mbedtls_mpi_uint) - 1) / sizeof(mbedtls_mpi_uint);
        } else if (grp->id <= MBEDTLS_ECP_DP_SECP521R1) {
            hsu_curve = HSU_EC_521;
            prime_limb = (66 + sizeof(mbedtls_mpi_uint) - 1) / sizeof(mbedtls_mpi_uint);
        }

        if (hsu_curve != HSU_EC_MAX)
        {
            const uint32_t *P_buf[2] = { P->X.p, P->Y.p };
            size_t P_len[2];
            uint32_t *R_buf[2];

            if (mbedtls_mpi_grow(&R->X, prime_limb) ||
            mbedtls_mpi_grow(&R->Y, prime_limb) ||
            mbedtls_mpi_lset(&R->Z, 1))
                goto no_hsu;

            P_len[0] = P->X.n * sizeof(mbedtls_mpi_uint) / sizeof(uint32_t);
            P_len[1] = P->Y.n * sizeof(mbedtls_mpi_uint) / sizeof(uint32_t);

            R_buf[0] = R->X.p;
            R_buf[1] = R->Y.p;

            if (hsu_ecp_mul(hsu_curve, (uint32_t *)m->p,
                    m->n * sizeof(mbedtls_mpi_uint) / sizeof(uint32_t),
                    P_buf, P_len, R_buf))
                return 0;
        }
    }
no_hsu:
    ...
    if (!restarting) {
        ...
        // LS:remove check on scalar value as in some case the scalar is not a
        // private key/ (e.g. in DPP protocol the scalar may be a message hash).
        // Removing the check remove the error, but the result is not always the
        // same as openSSL (the y coordinate is the "other" one).
        if (mbedtls_ecp_check_privkey( grp, m )) {
            work_around = true;
        }
...
cleanup:
    ...
    // LS:Work-around: when scalar is greater that curve's order (i.e.
    // mbedtls_ecp_check_privkey != 0) and the scalar is even mbedtls' ECP
    // multiplication give different result than OpenSSL => X coordinate is
    // the same but not the Y coordinate. As such recompute the other possible
    // Y coordinate for the given X (i.e. Y' = -Y mod P => Y' =  P - Y).
    if (work_around && !(m->p[0] & 0x1)) {
                                     ...
                                     mbedtls_ecp_restart_ctx *rs_ctx)
{
    ...
    if (mbedtls_mpi_cmp_int(m, 0) == 0) {
        ...
        // LS:this check will make wifi wpa3 sae mode fail
        // in mbedtls version 2.16.10, there is no this check
        // disable it here for wifi sae mode
        //MBEDTLS_MPI_CHK(mbedtls_ecp_check_pubkey(grp, P));
