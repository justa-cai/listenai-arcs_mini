...
// 本端内存管理
extern XosBlockPool lsf_parcelMemPool;
...
int LsfPropertyService_dispatch(LsfPropertyItem *property_item, LsfPropertyParcel *parcel)
{
    ...
    // LsfPropertyParcel *parcel = (LsfPropertyParcel *) pvPortMalloc(sizeof(LsfProperty_Parcel));
    ...
    if (LSF_PROPERTY_CONTEXT_TASK == property_item->context_type) {
        ...
        ret = xQueueSendToBack(property_item->context_msgQ, & msg, portMAX_DELAY);
...
int LsfPropertyService_execute(LsfPropertyItem *property_item, LsfPropertyParcel *parcel)
{
    ...
    case LSF_PROPERTY_FUNC_ID_SET:
        ...
        ret = LsfProperty_set(property, property_item->property_id, var);
        ...
        if ((property_item->flag & LSF_PROPERTY_FLAG_SYNCMODE_MSK) == LSF_PROPERTY_FLAG_SYNC) {
            ...
            // 注意: 允许用来传指针, 32位
            return_var.u.uiVal = (uint32_t) ret;
    ...
    case LSF_PROPERTY_FUNC_ID_GET:
        ...
        LsfProperty_get(property, property_item->property_id, var);
...
int LsfProperty_set(LsfProperty *self, int property_id, LsfVariant *var)
{
    CLOGD("[api] %s", __FUNCTION__);
    ...
    if (self->on_set != NULL) {
        // group_id 冗余信息 不传, property_id 传入.
        return self->on_set(0, property_id, var);
...
int LsfProperty_get(LsfProperty *self, int property_id, LsfVariant *var)
{
    CLOGD("[api] %s", __FUNCTION__);
                   ...
                   LsfVariant *var)
{
    ...
    if (var->variant_type == type_uint) {
        ...
        // 写入: marshal记录: type.
        _LsfPropertyService_push_frame_template.rpc.payload[8] = type_membuf;
    }
...
static void _LsfPropertyService_Set_In_handler(urpc_frame *frame)
{
    ...
    // 分包
    LsfPropertyParcel *parcel = (LsfPropertyParcel *) xos_block_try_alloc(&lsf_parcelMemPool);
