...
ICStreamCreator_createObj(ICStream *addr, int objID, struct ICStreamShare *remoteShare, int mode)
{
    ...
    if (ICSTREAM_MODE_CONSUMER == mode) {
        ...
        self->sema_consumerDataFrames = xSemaphoreCreateCounting(self->share->frameCount_InternalBuf,
                                                                 0);
        ...
        // TODO: 考虑到re-config可能出现的最大帧数, 取可能的最大数.
        // 注意: 若改成binary可避免此耦合, 但binary不支持同步用法. 老旧代码兼容性考虑. 暂不动.
        self->sema_producerEmptyFrames = xSemaphoreCreateCounting(self->share->frameCount_InternalBuf,
...
int ICStream_reconfig(ICStream *self)
{
    ...
    if (ICSTREAM_MODE_PRODUCER == self->mode) {
        ...
        UBaseType_t redisue = uxSemaphoreGetCount(self->sema_producerEmptyFrames);

        int difference = self->share->frameCount_InternalBuf - (int)redisue;

        while (difference > 0) {
            ret = xSemaphoreGive(self->sema_producerEmptyFrames);
            IC_ASSERT(ret == pdPASS);
            difference--;
        }
...
ICStream_Producer_waitFrame_timeOut(ICStream *self, int timeOutMs)
{
    ...
    // 等待"空帧数量" > 0
    ret = xSemaphoreTake(self->sema_producerEmptyFrames, pdMS_TO_TICKS(timeOutMs));
    // 超时返回错误码
    if (ret != pdPASS) {
        return IC_ERROR;
    }
