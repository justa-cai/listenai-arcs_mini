 ...
 * Those types are never dereferenced so they don't need to be fully defined (except
 * rtos_prio).
 *
 * The following MACROs must also be defined:
 * - @b RTOS_TASK_FCT(name):
 * Used to declare the task entry point function 'name'
 *
 * - @b RTOS_TASK_PRIORITY(prio):
 * Convert FHOST priority (int) in RTOS priority (rtos_prio). Higher value means higher
 * priority for the FHOST module.
 *
 ...
 * @param[in] new_size Size, in bytes, to reallocate.
 *
 * @return Address of reallocated memory on success and NULL if error occurred.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Create a RTOS task by static mode.
 *
 * @param[in] func Pointer to the task function
 * @param[in] name Name of the task
 * @param[in] task_id ID of the task
 ...
 * @param[out] task_handle Handle of the task, that might be used in subsequent RTOS
 *                         function calls
 * @param[in] task stack buffer pointer
 * @param[in] task stack control buffer pointer
 *
 * @return 0 on success and != 0 if error occurred.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Return RTOS run time, in millisecond
 *
 * @return run time
 ****************************************************************************************
 */
...
rtos_timer rtos_timer_create(void *id, bool reload, uint32_t period_ms, TimerCallbackFunction_t cb);
