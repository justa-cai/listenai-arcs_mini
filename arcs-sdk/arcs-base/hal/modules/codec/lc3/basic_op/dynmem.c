...
static void path_save(void)
{
    ...
    Max_Mem_path = (C_Path *) AUD_MALLOC(sizeof(C_Path));
    ...
    while(node != NULL){
        ...
        if(node->next != NULL) {
            node_save->next = (C_Path *) AUD_MALLOC(sizeof(C_Path));
        }
        node = node->next;
        previous = node_save;
...
void P_Dyn_Mem_Init(void)
{
    ...
    /* initialize the path */
    Head_path = (C_Path *) AUD_MALLOC(sizeof(C_Path));
                ...
                long mem_usage)
{
    ...
    Curr_path->next = (C_Path*) AUD_MALLOC (sizeof(C_Path));
...
void P_Sta_Mem_Init(void)
{
    ...
    Static_mem_head = (C_Path *) AUD_MALLOC(sizeof(C_Path));
...
{
    /* Enter a new function, push on the stack  */
    Static_mem_curr->next = (C_Path*) AUD_MALLOC (sizeof(C_Path));
