...
//    int v = CAMERA_READ_REG16(sensor->slv_addr, reg);
//    if(v < 0){
...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x, int pclk_root_div, bool pclk_manual, int pclk_div)
{
    ...
    CAMERA_LOGI(TAG"Calculated XVCLK: %d Hz, REFIN: %u Hz, VCO: %u Hz, PLL_CLK: %u Hz, SYSCLK: %u Hz, PCLK: %u Hz", xclk, REFIN, VCO, PLL_CLK, SYSCLK, PCLK);
    return SYSCLK;
}
...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x, uint8_t pclk_root_div, bool pclk_manual, uint8_t pclk_div){
    ...
    if(multiplier > 252 || multiplier < 4 || sys_div > 15 || pre_div > 8 || pclk_div > 31 || pclk_root_div > 3){
        CAMERA_LOGE(TAG"Invalid arguments");
        return -1;
    }
    if(multiplier > 127){
        multiplier &= 0xFE;//only even integers above 127
    }
    CAMERA_LOGI(TAG"Set PLL: bypass: %u, multiplier: %u, sys_div: %u, pre_div: %u, root_2x: %u, pclk_root_div: %u, pclk_manual: %u, pclk_div: %u", bypass, multiplier, sys_div, pre_div, root_2x, pclk_root_div, pclk_manual, pclk_div);
    ...
    if(ret){
        CAMERA_LOGE(TAG"set_sensor_pll FAILED!");
    }
    return ret;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if(framesize > FRAMESIZE_QSXGA){
        CAMERA_LOGE(TAG"Invalid framesize: %u", framesize);
        return -1;
    }
...
fail:
    ...
    CAMERA_LOGE(TAG"Setting framesize to: %ux%u failed", w, h);
    return ret;
}
...
static int set_agc_gain(sensor_t *sensor, int gain)
{
    ...
    if (ret == 0) {
        CAMERA_LOGD(TAG"Set agc_gain to: %d", gain);
...
static int set_aec_value(sensor_t *sensor, int value)
{
    ...
    if (max_val < 0) {
        CAMERA_LOGE(TAG"Could not read max aec_value");
        return -1;
    }
    if (value > max_val) {
        value =max_val;
    ...
    if (ret == 0) {
        CAMERA_LOGD(TAG"Set aec_value to: %d / %d", value, max_val);
...
static int set_ae_level(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        CAMERA_LOGD(TAG"Set ae_level to: %d", level);
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t level)
{
    ...
    if (ret == 0) {
        CAMERA_LOGD(TAG"Set gainceiling to: %d", l);
...
static int set_denoise(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        CAMERA_LOGD(TAG"Set denoise to: %d", level);
...
int ov5640_detect(int slv_addr, sensor_id_t *id)
{
    if (OV5640_SCCB_ADDR == slv_addr) {
        uint8_t h = CAMERA_READ_REG16(slv_addr, 0x300A);
        uint8_t l = CAMERA_READ_REG16(slv_addr, 0x300B);
        uint16_t PID = (h<<8) | l;
        if (OV5640_PID == PID) {
            id->PID = PID;
            return PID;
