...
)
{
    ...
    for (;;)                /* Parse JPEG segments */
    {
        ...
        //VIDEO_LOG("[%s:%d] marker=0x%x len=%d", __func__, __LINE__, marker, len);
...
__attribute__((aligned(4))) uint8_t jpg_buffer[JPEG_WBUF_SIZE];
...
static size_t jpeg_in_func(JDEC *jd, uint8_t *buf, size_t num)
{
    //VIDEO_LOG("buf=0x%x jpeg_data_size=%d jpeg_data_ptr=%d num=%d", buf, jd->jpeg_data_size, jd->jpeg_data_ptr, num);

    if (num > (jd->jpeg_data_size - jd->jpeg_data_ptr))
    {
        num = (jd->jpeg_data_size - jd->jpeg_data_ptr);
    }

    if (buf)
    {
        memcpy(buf, jd->jpeg_data + jd->jpeg_data_ptr, num);
    }

    jd->jpeg_data_ptr += num;
    return num;
}

static int jpeg_out_func(JDEC *jd, void *rgbbuf, JRECT *rect)
{
    uint16_t cnt = 0;
    uint16_t *pencolor = (uint16_t *)rgbbuf;
    uint16_t width = rect->right - rect->left + 1;  /* 填充的宽度 */
    ...
    VIDEO_LOG("rect: num=%d", num++);
    VIDEO_LOG("rect: left=%d right=%d", rect->left, rect->right);
    VIDEO_LOG("rect: top=%d bottom=%d", rect->top, rect->bottom);
#endif

    for (cnt = 0; cnt < height; cnt++)
    {
        rgb_data = jd->rgb_data + ((rect->top + cnt) * jd->width) + rect->left;
        memcpy(rgb_data, pencolor, width * sizeof(uint16_t));
        pencolor += width;
    }
...
uint8_t jpeg_to_rgb565(char *jpeg, uint32_t jpeg_size, uint16_t *rgb565)
{
    uint8_t res = 0;    /* 返回值 */

    memset(jpeg_dev, 0, sizeof (JDEC));
    jpeg_dev->jpeg_data = jpeg;
    jpeg_dev->jpeg_data_size = jpeg_size;
    jpeg_dev->rgb_data = rgb565;

    res = jd_prepare(jpeg_dev, jpeg_in_func, jpg_buffer, JPEG_WBUF_SIZE, NULL);
    if (res == JDR_OK)   /* 准备解码成功 */
    {
        VIDEO_LOG("jpeg width=%d height=%d", jpeg_dev->width, jpeg_dev->height);
    }
    else
    {
        VIDEO_LOG("jd_prepare error res=%d", res);
        return res;
    }

    res = jd_decomp(jpeg_dev, jpeg_out_func, 0);
    if (res == JDR_OK)   /* 解码成功 */
    {
        VIDEO_LOG("jpeg decode success");
    }
    else
    {
        VIDEO_LOG("jpeg decode error res=%d", res);
        return res;
    }
