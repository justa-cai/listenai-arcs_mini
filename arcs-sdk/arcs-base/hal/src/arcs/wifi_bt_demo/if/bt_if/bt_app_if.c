...
uint8_t app_ble_adv_start_handler(ble_adv_info_t *adv_info)
{
    ...
    switch(adv_info->adv_type)
    {
        ...
        case BLE_ADV_GEN_PAIRED :
        {
            ...
            #if (ADV_USER_DATA)
            adv_param.adv_param.gen_adv.user_data_len = sizeof(adv_user_data);
            adv_param.adv_param.gen_adv.user_data = (uint8_t*)adv_user_data;
            #else
            adv_param.adv_param.gen_adv.user_data_len = 0;
            #endif
            adv_param.adv_param.gen_adv.rsp_data_len = sizeof(APP_SCNRSP_DATA) - 1;
            adv_param.adv_param.gen_adv.rsp_data = (uint8_t*)APP_SCNRSP_DATA;
