...
HTTP_SESSION_HANDLE  HTTPClientOpenRequest (HTTP_CLIENT_SESSION_FLAGS Flags)
{
        ...
        // Init SSL net fd
        pHTTPSession->HttpSSL.m_httpc_net_fd.fd = -1;
...
UINT32 HTTPClientSendRequest (HTTP_SESSION_HANDLE pSession,
                ...
                HTTP_CLIENT_GET_HEADER GetHeader,      //  [IN] Get HTTP Header Function
                HTTP_CLIENT_GET_POST_DATA GetPostData) //  [IN] Get Post Data Function
{
                ...
                if(pHTTPSession->HttpCredentials.CredAuthSchema != AuthSchemaNone ||
...
#endif
                        {
                                ...
                                else
                                {
                                        ...
                                        {
                                                // Send the data
                                                nBytes = ((GetPostData == NULL)? nDataLength:strlen((CHAR*)pData));
                                                if((nRetCode = HTTPIntrnSend(pHTTPSession,pData,&nBytes)) != HTTP_CLIENT_SUCCESS)
                                                {
                                                        HC_ERR(("Send Header failed,%d",__LINE__));
                                                        break;
                                                }
                                                // Get External Post Data
                                                if (GetPostData != NULL)
                                                {
                                                        PostData postData = {.pData = NULL, .pLength = 0};
                                                        do
                                                        {
                                                                postData = GetPostData();
                                                                if(postData.pData != NULL && postData.pLength > 0)
                                                                {
                                                                        nBytes = postData.pLength;
                                                                        if((nRetCode = HTTPIntrnSend(pHTTPSession,postData.pData,&nBytes)) != HTTP_CLIENT_SUCCESS)
                                                                        {
                                                                                HC_ERR(("Send Post Data failed,%d",__LINE__));
                                                                                return nRetCode;
                                                                        }
                                                                } else if (postData.pLength < 0) {
																	nRetCode = HTTP_CLIENT_ERROR_SOCKET_SEND;
																	return nRetCode;
																}
                                                        } while(postData.pData != NULL && postData.pLength != 0);
                                                }
                                                else
                                                {
                                                        HC_DBG(("No External Post Data, %d", __LINE__));
                                                }
                ...
                UINT32 nUrlLength)  // [IN] The length the Url string
{
                                ...
                                &pUrlPtr->UrlPort) == TRUE)
        {
                if((pUrlPtr->UrlHost.nLength - pUrlPtr->UrlPort.nLength) < 10)
                {
                        ...
                        // Add string end to hostname
                        pUrlPtr->UrlHost.pParam[pUrlPtr->UrlHost.nLength - pUrlPtr->UrlPort.nLength - 1] = '\0';
                }
                else
                {
                        // Port too big
...
UINT32 HTTPIntrnConnectionClose (P_HTTP_SESSION pHTTPSession)
{
        ...
        do
        {
                ...
                if(pHTTPSession->HttpConnection.HttpSocket != HTTP_INVALID_SOCKET)// INVALID_SOCKET
                {
                        ...
                        {
                                // TLS Close
                                nRetCode = HTTPWrapperSSLClose(&(pHTTPSession->HttpSSL), pHTTPSession->HttpConnection.HttpSocket);
                        }
                        // Gracefully close it
#ifdef _WIN32
...
UINT32 HTTPIntrnConnectionOpen (P_HTTP_SESSION pHTTPSession)
{
        ...
        do
        {
                ...
                { // Is it a TLS connection?
                        ...
                        nRetCode = HTTPWrapperSSLConnect(
                                        &(pHTTPSession->HttpSSL),
                                        pHTTPSession->HttpConnection.HttpSocket,	// Socket
                ...
                {
                        if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SECURE) == HTTP_CLIENT_FLAG_SECURE){
                                HTTPWrapperSSLSetRecvTimeout(&(pHTTPSession->HttpSSL), pHTTPSession->HttpCounters.nActionTimeout * 1000);
                        }
        ...
        do
        {
                ...
                while(1)
                {
						...
						if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SECURE) == HTTP_CLIENT_FLAG_SECURE)
						{
								...
								if((nRetCode = HTTPWrapperSSLRecv(&(pHTTPSession->HttpSSL), pConnection->HttpSocket,pData,*(nLength),0)) == SOCKET_ERROR)
								{
										// Socket error
