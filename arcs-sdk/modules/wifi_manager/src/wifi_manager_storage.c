...
static wifi_storage_item_t *wifi_storage_get_info_list(wifi_storage_ctx_t *ctx, uint32_t info_num, uint32_t alloca_num)
{
    if(alloca_num == 0){
        return NULL;
    }

    wifi_storage_item_t *info_list = WIFI_STORAGE_CALLOC(ctx, alloca_num, sizeof(wifi_storage_item_t));
    if(info_list == NULL){
    ...
    if(info_num > 0){
        ...
        int len = 0;
        int r;
        r = lisa_kv_get_blob("wifi-list", (uint8_t **)&p, &len);
        if (r != 0 || len != info_num * sizeof(wifi_storage_item_t)) {
            LISA_LOGE(TAG, "Add item failed, nvs_read len (%d) is not equal to list len (%ld)", len,
                      info_num * sizeof(wifi_storage_item_t));
            WIFI_STORAGE_FREE(ctx, info_list);
            return NULL;
        }
        memcpy(info_list, p, len);
        lisa_kv_free(p);
    }
                                    ...
                                    const wifi_mgr_sta_config_t *item)
{
    ...
    lisa_kv_set_blob("wifi-list", (uint8_t *)info_list, (info_num) * sizeof(wifi_storage_item_t));
                                    ...
                                    const wifi_mgr_sta_config_t *item)
{
    ...
    WIFI_STORAGE_MUTEX_LOCK(ctx);
    lisa_kv_set_int("wifi-info-count", *info_num);
    lisa_kv_set_blob("wifi-list", (uint8_t *)info_list, (*info_num) * sizeof(wifi_storage_item_t));
                                    ...
                                    const wifi_mgr_sta_config_t *item)
{
    ...
    for (int idx = 0; idx < info_num; idx++) {
        if (strcmp(info_list[idx].ap_info.ssid, item->ssid) == 0 && 
            strcmp(info_list[idx].ap_info.bssid, item->bssid) == 0) {
            found = true;
    ...
    WIFI_STORAGE_MUTEX_LOCK(ctx);
    lisa_kv_set_blob("wifi-list", (uint8_t *)info_list, (int)((info_num) * sizeof(wifi_storage_item_t)));
    WIFI_STORAGE_MUTEX_UNLOCK(ctx);
...
int wifi_storage_init(wifi_storage_ctx_t *ctx, wifi_storage_ops_t *ops, void *mutex)
{
    ...
    WIFI_STORAGE_MUTEX_LOCK(ctx);
    lisa_kv_get_int("wifi-info-count", (int *)&ctx->list_item_count);
    WIFI_STORAGE_MUTEX_UNLOCK(ctx);
                           ...
                           wifi_mgr_storage_search_mode_t search_modes, void* target)
{
    ...
    wifi_storage_item_t *wifi_info_list = wifi_storage_get_info_list(ctx, info_num, info_num);
...
__out:
    if (hit_count == 0) {
        *matched_list = NULL;
        WIFI_STORAGE_FREE(ctx, wifi_info_list);
        return 0;
    }
...
int wifi_storage_save_ap(wifi_storage_ctx_t *ctx, wifi_mgr_sta_config_t *ap_info)
{
    ...
    int count = wifi_storage_search_ap(ctx, &matched_list, SEARCH_BY_SSID, ap_info->ssid);

    if (count > 0) {
        LISA_LOGE(TAG, "Item has been saved in the WiFi NVS Storage, ssid: %s, pwd: %s, bssid: %s",
    ...
    WIFI_STORAGE_MUTEX_LOCK(ctx);
    uint32_t current_count = ctx->list_item_count;
    WIFI_STORAGE_MUTEX_UNLOCK(ctx);
    
    wifi_storage_item_t *wifi_info_list = wifi_storage_get_info_list(ctx, current_count,
                                                                    current_count + 1);
    if (wifi_info_list == NULL) {
        ret = -ENOENT;
        goto __out;
    }
    wifi_storage_add_item(ctx, wifi_info_list, &current_count, ap_info);
    WIFI_STORAGE_FREE(ctx, wifi_info_list);
...
int wifi_storage_save_ap_force(wifi_storage_ctx_t *ctx, wifi_mgr_sta_config_t *ap_info)
{
    ...
    if (count > 0) {
        ...
        wifi_storage_item_t *wifi_info_list = wifi_storage_get_info_list(ctx, current_count, current_count);
        if (wifi_info_list == NULL) {
            ret = -ENOENT;
            goto __out;
        }
        ret = wifi_storage_replace_item(ctx, wifi_info_list, ap_info);
        WIFI_STORAGE_FREE(ctx, wifi_info_list);
        goto __out;
    } else if (count < 0) {
        ret = count;
...
int wifi_storage_delete_ap(wifi_storage_ctx_t *ctx, wifi_mgr_sta_config_t *ap_info)
{
    ...
    int count = wifi_storage_search_ap(ctx, &matched_list, SEARCH_BY_SSID, ap_info);
    ...
    if (count != 1) {
        if (count == 0) {
            ...
            LISA_LOGE(TAG,"Driver err, ssid: %s, pwd: %s, bssid: %s", ap_info->ssid, ap_info->pwd, ap_info->bssid);
            ret = -EINVAL;
            goto __out;
        } else {
            LISA_LOGE(TAG,"Unknown err, ssid: %s, pwd: %s, bssid: %s", ap_info->ssid, ap_info->pwd, ap_info->bssid);
            ret = -EINVAL;
            goto __out;
        }
    }
    ...
    wifi_storage_remove_item(ctx, wifi_info_list, info_num, ap_info);
    WIFI_STORAGE_FREE(ctx, wifi_info_list);
