...
void array_conf_init(ArrayConf *conf)
{
    ...
    conf->mem_alloc  = CC_MALLOC;
    conf->mem_calloc = CC_CALLOC;
    conf->mem_free   = CC_FREE;
}
...
void *array_get_for(Array *ar, size_t index)
{
	enum cc_stat stat;
	void *out;
    if (index == -1)
	...
	stat = array_get_at(ar, index, &out);
	if (stat != CC_OK)
		return NULL;
	return out;
}
...
Array *array_copy_shallow_ret(Array *ar)
{
    if(!ar)
    {
        return NULL;
	...
	Array *out;
	enum cc_stat state;
	state = array_copy_shallow(ar, &out);
	if (state != CC_OK)
		return NULL;
	return out;
}
