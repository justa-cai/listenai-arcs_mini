...
static void lv_refr_join_area(void)
{
    ...
    for(join_in = 0; join_in < disp_refr->inv_p; join_in++) {
        ...
        for(join_from = 0; join_from < disp_refr->inv_p; join_from++) {
            ...
            /* Force join all areas by calculating bounding box */
            joined_area.x1 = LV_MIN(disp_refr->inv_areas[join_in].x1, disp_refr->inv_areas[join_from].x1);
            joined_area.y1 = LV_MIN(disp_refr->inv_areas[join_in].y1, disp_refr->inv_areas[join_from].y1);
            joined_area.x2 = LV_MAX(disp_refr->inv_areas[join_in].x2, disp_refr->inv_areas[join_from].x2);
            joined_area.y2 = LV_MAX(disp_refr->inv_areas[join_in].y2, disp_refr->inv_areas[join_from].y2);
            
            /* Copy the merged area */
