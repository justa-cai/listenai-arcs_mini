...
void /* LV_ATTRIBUTE_FAST_MEM */ fill_normal(lv_color_t * dest_buf, const lv_area_t * dest_area,
...
#if LV_DRAW_COMPLEX
void fill_blended(lv_color_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride, lv_color_t color,
...
void /* LV_ATTRIBUTE_FAST_MEM */ map_normal(lv_color_t * dest_buf, const lv_area_t * dest_area,
...
#if LV_DRAW_COMPLEX
void map_blended(lv_color_t * dest_buf, const lv_area_t * dest_area, lv_coord_t dest_stride,
                        const lv_color_t * src_buf, lv_coord_t src_stride, lv_opa_t opa,
                                                  ...
                                                  const lv_draw_sw_blend_dsc_t * dsc)
{
    ...
    int ret = -1;
    bool if_big_area = lv_area_get_size(&blend_area) > CONFIG_LV_USE_RPC_THRESHOLD ? true : false;
#endif
...
#if CONFIG_LV_USE_RPC && (CONFIG_HARTID == 1)
            if(if_big_area){
                ret = lv_rpc_fill_normal(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa, mask, mask_stride);
            }
...
#if CONFIG_LV_USE_RPC && (CONFIG_HARTID == 1)
            if(if_big_area){
                ret = lv_rpc_map_normal(dest_buf, &blend_area, dest_stride, src_buf, src_stride, dsc->opa, mask, mask_stride);
            }
...
#if CONFIG_LV_USE_RPC && (CONFIG_HARTID == 1)
            if(if_big_area){
                ret = lv_rpc_fill_blended(dest_buf, &blend_area, dest_stride, dsc->color, dsc->opa, mask, mask_stride, dsc->blend_mode);
            }
...
#if CONFIG_LV_USE_RPC && (CONFIG_HARTID == 1)
            if(if_big_area){
                ret = lv_rpc_map_blended(dest_buf, &blend_area, dest_stride, src_buf, src_stride, dsc->opa, mask, mask_stride, dsc->blend_mode);
            }
...
LV_ATTRIBUTE_FAST_MEM void fill_normal(lv_color_t * dest_buf, const lv_area_t * dest_area,
                                              lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa,
                                              const lv_opa_t * mask, lv_coord_t mask_stride)
{
...
#if LV_DRAW_COMPLEX
void fill_blended(lv_color_t * dest_buf, const lv_area_t * dest_area,
