...
#define TAG "MemoryPool"

#ifdef __CONFIG_PSRAM_MEMPOOL
#define MALLOC(size) psram_malloc(size)
#define FREE(ptr) psram_free(ptr)
#else
#define MALLOC(size) lisa_mem_alloc(size)
#define FREE(ptr) lisa_mem_free(ptr)
#endif
...
#define MP_LOCK(lockobj)                                        \
	do {                                                        \
        lisa_mutex_lock(lockobj->lock, LISA_OS_WAIT_FOREVER);  \
	} while (0)
#define MP_UNLOCK(lockobj)                       \
	do {                                         \
        lisa_mutex_unlock(lockobj->lock);       \
	} while (0)
...
void
get_memory_info(
		MemoryPool *mp, _MP_Memory *mm, mem_size_t *free_list_len, mem_size_t *alloc_list_len)
{
#ifdef _Z_MEMORYPOOL_THREAD_
	MP_LOCK(mp);
#endif
...
{
	LISA_LOGI(TAG, "MemoryPool increase %lld size", new_mem_sz);
...
MemoryPool *
MemoryPoolInit(mem_size_t maxmempoolsize, mem_size_t mempoolsize)
{
...
#ifdef _Z_MEMORYPOOL_THREAD_
    mp->lock = lisa_mutex_create();
	if (mp->lock == NULL) {
		LISA_LOGE(TAG, "create mutex failed!");
		FREE(mp);
		return NULL;
	}
#endif
...
void *
MemoryPoolAlloc(MemoryPool *mp, mem_size_t wantsize)
{
...
FIND_FREE_CHUNK:
		...
		while (_free) {
			if (_free->alloc_mem >= total_needed_size) {
				...
				if (_free->alloc_mem - total_needed_size > MP_CHUNKHEADER + MP_CHUNKEND) {
					...
					*(_MP_Chunk **)((char *)_not_free + total_needed_size - MP_CHUNKEND) =
							_not_free;
				}
				// 不够 则整块分配为alloc
				else {
	...
	if (mp->auto_extend) {
		...
		if (mp->total_mem_pool_size >= mp->max_mem_pool_size) {
			LISA_LOGE(TAG, "Memory has exceeded the max limit %lld", mp->max_mem_pool_size);
			goto err_out;
		}
...
int
MemoryPoolFree(MemoryPool *mp, void *p)
{
...
int
MemoryPoolDestroy(MemoryPool *mp)
{
	if (mp == NULL) return 1;
#ifdef _Z_MEMORYPOOL_THREAD_
	MP_LOCK(mp);
...
mem_size_t
GetTotalMemory(MemoryPool *mp)
{
	return mp->total_mem_pool_size;
}

mem_size_t
GetUsedMemory(MemoryPool *mp)
{
#ifdef _Z_MEMORYPOOL_THREAD_
	MP_LOCK(mp);
#endif
...
float
MemoryPoolGetUsage(MemoryPool *mp)
{
	return (float)GetUsedMemory(mp) / GetTotalMemory(mp);
}

float
MemoryPoolGetProgUsage(MemoryPool *mp)
{
