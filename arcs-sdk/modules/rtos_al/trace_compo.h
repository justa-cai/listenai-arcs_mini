...
/**
 *****************************************************************************
 * Specific filter for RUA
 *****************************************************************************
 */
enum trace_level_rua
{
    /** Error */
    TRACE_LVL_RUA_ERR,
    /** Trace all frame sent in OFDMA mode and high level scheduling of the algorithm */
    TRACE_LVL_RUA_TX,
    /** Trace all frame received in OFDMA mode */
    TRACE_LVL_RUA_RX,
    /** Trace STA buffer content */
    TRACE_LVL_RUA_CHECK_BUFF,
    /** Trace STA Statistic buffer content */
    TRACE_LVL_RUA_STAT,
    /** Trace MAP sent to the driver */
    TRACE_LVL_RUA_MAP,
    /** Trace hungarian algorithm, lowest level */
    TRACE_LVL_RUA_HUNG_INNER_LVL0,
    /** Trace hungarian algorithm, highest level */
    TRACE_LVL_RUA_HUNG_INNER_LVL1,
    /** Trace calls related to the memory mode of the hungarian algorithm */
    TRACE_LVL_RUA_HUNG_MEMORY,
};

/** Default trace level for RUA component */
#define TRACE_RUA_DEFAULT (CO_BIT(TRACE_LVL_RUA_ERR) | \
                           CO_BIT(TRACE_LVL_RUA_MAP))


/** Define whether to include or not RUA traces */
#define TRACE_RUA_EN 1
#if TRACE_COMPO(RUA)
/** add trace point for RUA component */
#define TRACE_RUA(lvl, ...) TRACE_FILT(RUA, RUA_##lvl, __VA_ARGS__)
/** add buffer trace point for RUA component */
#define TRACE_BUF_RUA(lvl, ...) TRACE_BUF_FILT(RUA, RUA_##lvl, __VA_ARGS__)
/** Test whether RUA level is currently enabled or not */
#define TRACE_RUA_ON(lvl) TRACE_FILT_ON(RUA, RUA_##lvl)
#else
#define TRACE_RUA(lvl, ...)
#define TRACE_BUF_RUA(lvl, ...)
#define TRACE_RUA_ON(lvl) 0
#endif /* TRACE_COMPO(RUA) */
