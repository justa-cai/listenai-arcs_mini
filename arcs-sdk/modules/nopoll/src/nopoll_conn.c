							...
							noPollConnOpts  * options)
{
    ...
    int keepIdle = 30;    // 如果在30秒内没有任何数据交互,则进行探测. 缺省值:7200(s)
    int keepInterval = 1; // 探测时发探测包的时间间隔为1秒. 缺省值:75(s)
    int keepCount = 10;    // 探测重试的次数. 全部超时则认定连接失效..缺省值:9(次)
    setsockopt(session, SOL_SOCKET, SO_KEEPALIVE, (void *)&keepAlive, sizeof(keepAlive));
    setsockopt(session, IPPROTO_TCP, TCP_KEEPIDLE, (void *)&keepIdle, sizeof(keepIdle));
    setsockopt(session, IPPROTO_TCP, TCP_KEEPINTVL, (void *)&keepInterval, sizeof(keepInterval));
    setsockopt(session, IPPROTO_TCP, TCP_KEEPCNT, (void *)&keepCount, sizeof(keepCount));
    /* disable nagle */
...
int nopoll_conn_tls_receive (noPollConn * conn, char * buffer, int buffer_size)
{
    ...
    if (ret < 0 && ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE && ret != MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY){
		//可以表示网络异常
		printf("liangzhu--->nopoll_conn_tls_receive connection error return %d\n", ret);
		return NOPOLL_SOCKET_ERROR;
	}
    // printf("liangzhu--->nopoll_conn_tls_receive returned %d errno: %d\n", ret, errno);
    return ret;
#else
	int res;
				       ...
				       const char      * origin)
{
	...
	if (enable_tls) {
		...
		if (options && options->private_key) {
#ifdef MBEDTLS_V3
			if (mbedtls_pk_parse_key(conn->ssl_pkey, (const unsigned char *)options->private_key, options->private_key_size, NULL, 0, mbedtls_ctr_drbg_random, conn->ssl_ctr_drbg ) != 0)
			{
		...
		while ((ssl_error = mbedtls_ssl_handshake(conn->ssl)) != 0) {
			...
			if ((ret = mbedtls_ssl_get_verify_result(conn->ssl)) != 0) {
				...
				char *vrfy_buf = nopoll_calloc(MBEDTLS_VRFY_BUF_SIZE, sizeof(char));
				if (!vrfy_buf) {
...
noPollConn * nopoll_conn_new_with_ctrl (noPollCtx  * ctx,
			      const char * host_ip,
			      const char * host_port,
			      const char * host_name,
			      const char * get_url,
				  ...
				  const int32_t *ctrl)
{
	...
	return nopoll_conn_new(ctx, host_ip, host_port, host_name, get_url, protocols, origin);
}
...
noPollConn * nopoll_conn_tls_new_with_ctrl (noPollCtx  * ctx,
				  noPollConnOpts  * options,
				  const char * host_ip,
				  const char * host_port,
				  const char * host_name,
				  ...
				  const int32_t *ctrl)
{
	...
	return nopoll_conn_tls_new(ctx, options, host_ip, host_port, host_name, get_url, protocols, origin);
}
...
#ifdef MBEDTLS_V3
		if (mbedtls_pk_parse_key(conn->ssl_pkey, (const unsigned char *) privateKey, privateKey_size, NULL, 0, mbedtls_ctr_drbg_random, conn->ssl_ctr_drbg ) != 0) {
