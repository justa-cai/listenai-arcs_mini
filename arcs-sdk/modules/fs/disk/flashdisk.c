...
#include "flash_arcs.h"

static sflash_dev_t sflash_dev = {
	.flash_dev = NULL,
};

/* flash read-copy-erase-write operation */
static uint8_t __aligned(4) read_copy_buf[CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE];
...
static uint32_t lba_to_address(uint32_t sector_num)
{
	uint32_t flash_addr;

	flash_addr = CONFIG_DISK_FLASH_START_ADDR + sector_num * SECTOR_SIZE;
								  ...
								  uint32_t start_sector, uint32_t sector_count)
{
	...
	src_addr = (uint8_t*)CONFIG_DISK_FLASH_CHIP_BASE_ADDR + fl_addr;
	memcpy(buff, src_addr, sector_count*SECTOR_SIZE);
								 ...
								 uint8_t *dest_buff)
{
	...
	num_read = GET_NUM_BLOCK(CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE,
 ...
 * CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE.
 */
static int update_flash_block(off_t start_addr, uint32_t size, const void *buff)
{
	...
	if (size < CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE)
	{
		int rc;
	...
	if (sflash_erase(&sflash_dev, fl_addr, CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE) != 0)
	{
		return -EIO;
	}

	/* write data to flash */
	num_write = GET_NUM_BLOCK(CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE,
							  CONFIG_DISK_FLASH_MAX_RW_SIZE);

	for (uint32_t i = 0; i < num_write; i++)
	{
		if (sflash_write(&sflash_dev, fl_addr, src,
								   ...
								   uint32_t start_sector, uint32_t sector_count)
{
	...
	if (fl_addr & (CONFIG_DISK_FLASH_ERASE_ALIGNMENT - 1))
	{
		...
		block_bnd = fl_addr + CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE;
		block_bnd = block_bnd & ~(CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE - 1);
	...
	while (remaining)
	{
		...
		if (remaining < CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE)
		{
			break;
		}

		rc = update_flash_block(fl_addr, CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE,
								buff);
		if (rc != 0)
		{
			return -EIO;
		...
		fl_addr += CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE;
		remaining -= CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE;
		buff += CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE;
	}
...
static int disk_flash_access_ioctl(struct disk_info *disk, uint8_t cmd, void *buff)
{
	...
	case DISK_IOCTL_GET_SECTOR_COUNT:
		*(uint32_t *)buff = CONFIG_DISK_FLASH_VOLUME_SIZE / SECTOR_SIZE;
		return 0;
	case DISK_IOCTL_GET_SECTOR_SIZE:
		*(uint32_t *)buff = SECTOR_SIZE;
		return 0;
	case DISK_IOCTL_GET_ERASE_BLOCK_SZ: /* in sectors */
		*(uint32_t *)buff = CONFIG_DISK_FLASH_ERASE_BLOCK_SIZE / SECTOR_SIZE;
		return 0;
	default:
		break;
	}
