 ...
 *   STATIC FUNCTIONS
 **********************/
#if CONFIG_LV_CUSTOM_MONITOR && CONFIG_LV_CUSTOM_TASK_CPU_PER
#include "log_print.h"
#include "FreeRTOS.h"
#include "task.h"
...
static bool lv_task_exec(lv_task_t * task)
{
    ...
    if(lv_task_time_remaining(task) == 0) {
        ...
        task_created   = false;
#if CONFIG_LV_CUSTOM_MONITOR && CONFIG_LV_CUSTOM_TASK_CPU_PER
        uint32_t start = ulTaskGetRunTimeCounter(task_ui);
#endif
        if(task->task_cb) task->task_cb(task);
#if CONFIG_LV_CUSTOM_MONITOR && CONFIG_LV_CUSTOM_TASK_CPU_PER
        CLOGI("(%p)cpu cost:%d", *((void **)&task->task_cb), ulTaskGetRunTimeCounter(task_ui) - start);
        CLOGI("(%p)cost %dms", *((void **)&task->task_cb), lv_tick_get() - task->last_run);
#endif
