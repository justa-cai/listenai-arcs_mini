...
#if LV_USE_GPU_STM32_DMA2D || LV_USE_GPU_CSK_DMA2D
                LV_UNUSED(copy_buf);
#else
                copy_buf = _lv_mem_buf_get(lv_disp_get_hor_res(disp_refr) * sizeof(lv_color_t));
#endif
...
#elif LV_USE_GPU_CSK_DMA2D && CONFIG_LVGL_GPU_CSK_COPY
                        if (((uint32_t)(buf_act + start_offs) % 4 == 0) && (lv_area_get_width(&disp_refr->inv_areas[a]) % 2 == 0)) {
                            lv_gpu_csk_dma2d_copy((lv_color_t *)(buf_act + start_offs), hres,
                                                    (lv_color_t *)(buf_ina + start_offs), hres,
                                                    lv_area_get_width(&disp_refr->inv_areas[a]),
                                                    lv_area_get_height(&disp_refr->inv_areas[a]));
                        }
#else
...
#if CONFIG_LV_CUSTOM_TASK_CPU_PER
        extern uint8_t lvgl_busy;
        cpu = lvgl_busy;
#endif
...
#ifdef CONFIG_LV_REFR_UNCONDITIONAL_AREA_JOIN
            /* Unconditionally join all areas, regardless of whether they overlap */
