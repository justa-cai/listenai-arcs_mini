...
LV_ATTRIBUTE_FAST_MEM void fill_normal(const lv_area_t * disp_area, lv_color_t * disp_buf,
...
LV_ATTRIBUTE_FAST_MEM void map_normal(const lv_area_t * disp_area, lv_color_t * disp_buf,
                                             const lv_area_t * draw_area,
                                             const lv_area_t * map_area, const lv_color_t * map_buf, lv_opa_t opa,
                                             const lv_opa_t * mask, lv_draw_mask_res_t mask_res);
...
#if LV_USE_GPU || LV_USE_GPU_STM32_DMA2D || (LV_USE_GPU_CSK_DMA2D && CONFIG_LVGL_GPU_CSK_BLEND)
    #include "sysutils.h"
    __psram_bss__ static lv_color_t blend_buf[LV_HOR_RES_MAX];
#endif
...
#if LV_USE_GPU_CSK_DMA2D && CONFIG_LVGL_GPU_CSK_FILL
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER && opa > LV_OPA_MAX
        && lv_area_get_size(draw_area) > CONFIG_LVGL_GPU_CSK_FILL_MIN_SIZE){
        int ret = lv_gpu_csk_dma2d_fill(disp_buf_first, disp_w, color, draw_area_w, draw_area_h);
        if(ret == 0){
            return;
        }
    }
...
//工程中只有C核会执行代理
#if CONFIG_LV_USE_RPC && (CONFIG_HARTID == 1)
    if(lv_area_get_size(draw_area) >= CONFIG_LV_USE_RPC_THRESHOLD){
        int ret = lv_rpc_fill_normal(disp_area, disp_buf, draw_area, color, opa, mask, mask_res);
        if(ret == 0) {
            return;
        }
    }
...
#elif LV_USE_GPU_CSK_DMA2D && CONFIG_LVGL_GPU_CSK_BLEND
            if(lv_area_get_size(draw_area) >= CONFIG_LVGL_GPU_CSK_BLEND_MIN_SIZE && 
              (LV_HOR_RES_MAX >= draw_area_w) &&
              ((uint32_t)disp_buf_first % 4 == 0) &&
              ((uint32_t)draw_area_w % 2 == 0)) {
                if(blend_buf[0].full != color.full) lv_color_fill(blend_buf, color, LV_HOR_RES_MAX);

                lv_coord_t line_h = LV_HOR_RES_MAX / draw_area_w;
                for(y = 0; y <= draw_area_h - line_h; y += line_h) {
                    lv_gpu_csk_dma2d_blend(disp_buf_first, disp_w, blend_buf, opa, draw_area_w, draw_area_w, line_h);
                    disp_buf_first += disp_w * line_h;
                }

                if(y == draw_area_h) {
                    return;
                }else if( draw_area_h - y  != 1) {
                    lv_gpu_csk_dma2d_blend(disp_buf_first, disp_w, blend_buf, opa, draw_area_w, draw_area_w, draw_area_h - y);
                    return;
                }else{
                    draw_area_h = 1;
                }
...
#elif LV_USE_GPU_CSK_DMA2D && CONFIG_LVGL_GPU_CSK_FILL_MASK
        if(lv_area_get_size(draw_area) > CONFIG_LVGL_GPU_CSK_FILL_MASK_MIN_SIZE  && 
          ((uint32_t)disp_buf_first % 4 == 0) 
          && ((uint32_t)draw_area_w % 4 == 0)) {
            lv_gpu_csk_dma2d_fill_mask(disp_buf_first, disp_w, color, mask, opa, draw_area_w, draw_area_h);
            return;
        }
#endif
...
#if LV_USE_GPU_CSK_DMA2D && CONFIG_LVGL_GPU_CSK_COPY
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER && opa > LV_OPA_MAX
        && lv_area_get_size(draw_area) > CONFIG_LVGL_GPU_CSK_COPY_MIN_SIZE){
        int ret = lv_gpu_csk_dma2d_copy(disp_buf_first, disp_w, map_buf_first, map_w, draw_area_w, draw_area_h);
        if(ret == 0){
            return;
        }
    }
...
#if CONFIG_LV_USE_RPC && (CONFIG_HARTID == 1)
    if(lv_area_get_size(draw_area) >= CONFIG_LV_USE_RPC_THRESHOLD){
        int ret = lv_rpc_map_normal(disp_area, disp_buf, draw_area, map_area, map_buf, opa, mask, mask_res);
        if(ret == 0) {
            return;
        }
    }
    ...
    if(mask_res == LV_DRAW_MASK_RES_FULL_COVER) {
        ...
        if(opa > LV_OPA_MAX) {
            ...
            if(lv_area_get_size(draw_area) >= CONFIG_LVGL_GPU_CSK_BLEND_MIN_SIZE  && 
             ...
             (draw_area_w % 4 == 0) && 
             (draw_area_h > 1)) {
                lv_gpu_csk_dma2d_blend(disp_buf_first, disp_w, map_buf_first, opa, map_w, draw_area_w, draw_area_h);
                return;
            }
#endif
