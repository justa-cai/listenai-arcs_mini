...
#if CONFIG_LV_CUSTOM_MONITOR
    uint64_t start_cycle = get_time_cycle();
#endif

    lv_res_t res;
    res = lv_img_draw_core(coords, mask, src, dsc);
#if CONFIG_LV_CUSTOM_MONITOR
    g_draw_image_time += calc_time_elapsed_cycle(start_cycle);
    g_draw_image_call_cnt += 1;
#endif
    if(res == LV_RES_INV) {
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        if(other_mask_cnt == 0 && !transform && !chroma_key && draw_dsc->recolor_opa == LV_OPA_TRANSP && alpha_byte) {
            ...
            if(lv_area_get_size(&draw_area) > CONFIG_LVGL_GPU_CSK_BLEND_MIN_SIZE) {
                ...
                lv_color_t * disp_buf = vdb->buf_act;
                lv_color_t * disp_buf_first = disp_buf + disp_w * draw_area.y1 + draw_area.x1;
                if (((uint32_t)disp_buf_first % 4 == 0) && 
                   (draw_area_w % 4 == 0) 
                   && (draw_area_h > 1)){
                    lv_gpu_csk_dma2d_blend(disp_buf_first, disp_w, (const lv_color_t *)map_buf_tmp, draw_dsc->opa, map_w, draw_area_w,
                                            draw_area_h);
                    return;
                }
            }
