...
int add_message_header_field(fisbone_packet *fp, char *header_key, char *header_value)
{
	...
	snprintf(fp->message_header_fields + fp->current_header_size, this_message_size + 1, "%s: %s\r\n", header_key,
		 header_value);
...
fisbone_packet fisbone_from_ogg(ogg_packet *op)
{
	...
	fp.serial_no =
		*((ogg_uint32_t *)(op->packet + 12)); /* serialno of the stream represented by this fisbone packet */
