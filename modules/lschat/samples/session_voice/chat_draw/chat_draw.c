...
static const unsigned char draw_inc_file[] = {
#include <draw.pcm.inc>
};
...
static void voice_event_cb(session_voice_event_e evt, void *data, uint32_t size, void *usr)
{
	...
	case SESSION_VOICE_DRAW:
		LISA_NLOGI("draw url:%s", (char *)data);
		break;
	default:
		break;
	}
...
static int mock_record_audio_stream_send(uint8_t *data, uint32_t size)
{
	...
	int ret = 0;
	do {
		send_size = (remain_size > 2560) ? 2560 : remain_size;
...
void chat_draw_proc(void)
{
	...
	ret = session_voice_add_evt_callback(
		voice_event_cb, SESSION_VOICE_TTS | SESSION_VOICE_IAT | SESSION_VOICE_REPLY_URL | SESSION_VOICE_DRAW,
		NULL);
	...
	ret = mock_record_audio_stream_send((uint8_t *)draw_inc_file, sizeof(draw_inc_file));
}
