...
概述
****
该示例程序介绍如何通过闲聊会话来让云端生成图片。将提前录音音频数据发送云端，云端进行处理后，生成图片下发url。
...
- 初始化lsc服务，并设置设备ID、产品ID、secret ID
  调用lsc_init()初始化lsc服务，并设置设备ID、产品ID、secret ID
...
- 启动会话，并上传语音数据
  ...
  .. note:: draw_inc_file[]数组中存放的是录音("画一张奥特曼的照片")数据，格式为16Khz, 16bit单通道PCM
