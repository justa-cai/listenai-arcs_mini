...
static const unsigned char lisa_base64_suffix_map[256] = {
...
unsigned char *lsc_base64_encode(unsigned char *str)
{
	...
	res = (unsigned char *)lisa_mem_calloc(1, sizeof(unsigned char) * len + 1);
	if (res == NULL) {
		return NULL;
	}
	res[len] = '\0';
	...
	// 以3个8位字符为一组进行编码
	for (i = 0, j = 0; j < str_len - 2; j += 3, i += 4) {
		res[i] = base64_table[str[j] >> 2]; // 取出第一个字符的前6位并找出对应的结果字符
		res[i + 1] = base64_table[(str[j] & 0x3) << 4 |
					  (str[j + 1] >> 4)]; // 将第一个字符的后2位与第二个字符的前4位组合
		res[i + 2] = base64_table[(str[j + 1] & 0xf) << 2 |
					  (str[j + 2] >> 6)]; // 将第二个字符的后4位与第三个字符的前2位组合
...
uint8_t *lsc_base64_encodev2(const uint8_t *data, uint32_t data_len)
{
	...
	int i, j;
	// 定义base64编码表
	uint8_t *base64_table = (uint8_t *)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	...
	res = (uint8_t *)lisa_mem_calloc(1, sizeof(uint8_t) * len + 1);
	if (res == NULL) {
		return NULL;
	}
	res[len] = '\0';
	...
	// 以3个8位字符为一组进行编码
	for (i = 0, j = 0; j < data_len - 2; j += 3, i += 4) {
