...
#include "../widgets/lv_wordbook.h"
#include "wordbook_pages.h"
#include "user_groups.h"

#include "lisaui_log.h"

#define TAG "wordbook.page.main"

typedef struct{
    lv_obj_t *display;
    lv_obj_t *wordbook;    /**< 组网格视图组件 */
}page_view_t;
...
static lisaui_page_t *create(lisaui_page_t *page){
    ...
    view->wordbook = lv_wordbook_create(view->display);
    page->view = view;

    LISAUI_LOGI(TAG,"Create page:%p,view->wordbook:%p",page,view->wordbook);
    return page;
...
static lisaui_err_t update_data(lisaui_page_t *page,void *data){
    ...
    wordbook_page_update_parameter_t *parameter = (wordbook_page_update_parameter_t*)data;
    if (!page) {
...
static const lisaui_page_t wordbook_main_page = {
    .group_index = LISAUI_GROUP_INDEX_WORDBOOK,
    .page_index = LISAUI_GROUP_WORDBOOK_PAGE_MAIN_INDEX,
    .cname = "wordbook.main",
    .view = NULL,
    .page_data = NULL,
    .attribute = 0,
...
LISAUI_PAGE_EXPORT(wordbook_main_page);
