...
#include "group_wordbook.h"
#include "lisaui_manager.h"
#include "platform.h"
#include "user_groups.h"

static const char *TAG = "group.wordbook";


static lisaui_err_t group_wordbook_setup(lisaui_group_t *group)
{
...
static lisaui_err_t group_wordbook_cleanup(lisaui_group_t *group)
{
    if(group == NULL){
        return LISAUI_ERR_APP_ID_INVALID;
    }
...
static lisaui_err_t group_wordbook_enter(lisaui_group_t *group,lisaui_group_enter_page_method_t method,int page_index)
{
    lisaui_page_t *page;
    lisaui_page_t *new_page;
    lisaui_err_t ret = LISAUI_ERR_OK;
    
    LISAUI_LOGI(TAG, "Entering wordbook group with method: %d, page_index: %d", method, page_index);
    ...
    if(method == GROUP_ENTER_PAGE_METHOD_FIX_PAGE_INDEX){
        ...
        if(NULL == page){
            ...
            if(page_index == LISAUI_GROUP_WORDBOOK_PAGE_MAIN_INDEX){
                LISAUI_LOGI(TAG, "Creating main page for wordbook group");
                new_page = lisaui_page_create(LISAUI_GROUP_INDEX_WORDBOOK,LISAUI_GROUP_WORDBOOK_PAGE_MAIN_INDEX);
                if(new_page == NULL){
        ...
        if(NULL == page){
            page = lisaui_page_create(LISAUI_GROUP_INDEX_WORDBOOK,LISAUI_GROUP_WORDBOOK_PAGE_MAIN_INDEX);
            if(page == NULL){
                LISAUI_LOGE(TAG, "Failed to create main page");
                ret = LISAUI_ERR_APP_PAGE_NOT_EXIST;
                goto _ERR;
    ...
    LISAUI_LOGI(TAG, "Successfully entered wordbook group");
    return LISAUI_ERR_OK;

_ERR:
    LISAUI_LOGE(TAG, "Failed to enter wordbook group, error code: %d", ret);
    return ret;   


}

static lisaui_err_t group_wordbook_exit(lisaui_group_t *group)
{
...
static int manager_event_cb(uint32_t events,lisaui_group_t* group,void* user_data){
    ...
    LISAUI_LOGI(TAG,"Lisaui manager events:0x%x,trigger by group:%s",events,group->info.name);
...
static lisaui_group_t group_wordbook = {
    .setup = group_wordbook_setup,
    .cleanup = group_wordbook_cleanup,
    .enter = group_wordbook_enter,
    .exit = group_wordbook_exit,
    .info =
        {
            .name = "lisaui.wordbook",
            .package_name = "com.listenai.lisaui.wordbook",
            .id = LISAUI_GROUP_INDEX_WORDBOOK,
...
static lisaui_err_t group_wordbook_init(void)
{
    LISAUI_LOGI(TAG,"%s",__FUNCTION__);
    lisaui_manager_group_register(&group_wordbook);
...
LISAUI_GROUP_REGISTER(group_wordbook,group_wordbook_init,2);
